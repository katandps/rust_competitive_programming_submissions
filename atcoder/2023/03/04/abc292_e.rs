...
# [rustfmt :: skip ] pub trait ToLR < T > : Clone {fn to_lr (& self ) -> (T , T ) ; }
# [rustfmt :: skip ] impl < R : RangeBounds < T > + Clone , T : Copy + BoundedAbove + BoundedBelow + One + Add < Output = T > > ToLR < T > for R {# [inline ] fn to_lr (& self ) -> (T , T ) {use Bound :: {Excluded , Included , Unbounded } ; let l = match self . start_bound () {Unbounded => T :: min_value () , Included (& s ) => s , Excluded (& s ) => s + T :: one () , } ; let r = match self . end_bound () {Unbounded => T :: max_value () , Included (& e ) => e + T :: one () , Excluded (& e ) => e , } ; (l , r ) } }
# [rustfmt :: skip ] pub trait RangeProduct < I > {type Magma : Magma ; fn product < R : ToLR < I > > (& self , range : R ) -> < Self :: Magma as Magma > :: M ; }
# [rustfmt :: skip ] pub trait RangeProductMut < I > {type Magma : Magma ; fn product < R : ToLR < I > > (& mut self , range : R ) -> < Self :: Magma as Magma > :: M ; }
...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    for (u, v) in uv {
        graph.add_arc(u - 1, v - 1, 1);
    }
    let deg = graph.outdegree();
    let mut ans = 0;
    for src in 0..n {
        ...
        while let Some(src) = q.pop_front() {
            for (dst, _) in graph.edges(src) {
                if chmax!(dist[dst], true) {
                    q.push_back(dst);
                }
            }
        }
        let cnt = dist.iter().filter(|&&b| b).count();
        ans += cnt - deg[src] as usize - 1;
    }
    io.out(ans.ln());
}
