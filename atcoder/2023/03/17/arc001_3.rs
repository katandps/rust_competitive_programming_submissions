use next_permutation_impl::next_permutation_from_i;
...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n = 8;
    let mut c = io.char_map(n);
    let mut queens = Vec::new();
    let mut rows = vec![true; n];
    let mut columns = vec![true; n];
    for y in 0..n {
        for x in 0..n {
            if c[y][x] == 'Q' {
                columns[x] = false;
                rows[y] = false;
                queens.push((y as i64, x as i64));
            }
        }
    }
    if rows.iter().filter(|&&b| b).count() != 5 || columns.iter().filter(|&&b| b).count() != 5 {
        return io.out("No Answer\n");
    }
    let mut rest_rows = Vec::new();
    let mut rest_columns = Vec::new();
    for i in 0..n {
        if rows[i] {
            rest_rows.push(i);
        }
    ...
    for yv in next_permutation_from_i(5) {
        for xv in next_permutation_from_i(5) {
            for i in 0..5 {
                queens.push((rest_rows[yv[i]] as i64, rest_columns[xv[i]] as i64));
            }
            if check(&queens) {
                for (y, x) in queens {
                    c[y as usize][x as usize] = 'Q';
                }
                return out(&c, io);
            }
            for _i in 0..5 {
...
fn check(queens: &[(i64, i64)]) -> bool {
    const N: usize = 8;
    for i in 0..N {
        let (yi, xi) = queens[i];
        for j in i + 1..N {
            let (yj, xj) = queens[j];
            if yi + xi == yj + xj || yi - xi == yj - xj {
                return false;
            }
        }
    }
...
fn out<IO: ReaderTrait + WriterTrait>(ans: &Vec<Vec<char>>, io: &mut IO) {
    for i in 0..8 {
        io.out(ans[i].iter().join("").line());
    }
}
