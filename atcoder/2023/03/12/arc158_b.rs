...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    for _ in 0..3 {
        if let Some(n) = negs.pop() {
            take.push(n);
        }
        if let Some(n) = poss.pop() {
            take.push(n);
        }
    }
    negs.reverse();
    ...
    for i in 0..n {
        for j in i + 1..n {
            for k in j + 1..n {
                let upper = take[i] + take[j] + take[k];
                let lower = take[i] * take[j] * take[k];
                chmin!(min, upper as f64 / lower as f64);
                chmax!(max, upper as f64 / lower as f64);
            }
        }
    }
    io.out(min.line());
