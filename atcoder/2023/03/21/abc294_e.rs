...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (_l, n1, n2) = io.v3::<usize, usize, usize>();
    let vl1 = io.vec2::<usize, usize>(n1);
    let vl2 = io.vec2::<usize, usize>(n2);
    dbg!(&vl1, &vl2);
    let mut ans = 0usize;
    let (mut rest_num, mut rest_cnt) = (0, 0);
    let mut i2 = 0;
    for i in 0..n1 {
        let (num, mut rest) = vl1[i];
        dbg!(num, rest, ans, rest_cnt);
