...
pub fn binary_search<F: Fn(f64) -> bool>(mut ok: f64, mut ng: f64, f: F) -> f64 {
    while (ok - ng).abs() > 0.000_000_000_001 {
        let mid = (ok + ng) / 2.0;
        if f(mid) {
            ok = mid
        } else {
...
impl<T: PartialOrd> SliceBounds for [T] {
    type Item = T;
    fn lower_bound(&self, k: &T) -> usize {
        self.lower_bound_by(|p| p.partial_cmp(k).unwrap())
    }
    fn upper_bound(&self, k: &T) -> usize {
        self.upper_bound_by(|p| p.partial_cmp(k).unwrap())
    }
}
...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let ab = io.vec2::<i64, i64>(n);
    let cd = io.vec2::<i64, i64>(m);
    let ans = binary_search(100.0, 0.0, |x| {
        let p = ab
            .iter()
            .map(|(a, b)| *a as f64 - x * (*a + *b) as f64)
            .collect::<Vec<_>>();
        let mut q = cd
            .iter()
            .map(|(c, d)| *c as f64 - x * (*c + *d) as f64)
            .collect::<Vec<_>>();
        q.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let mut cnt = 0;
        for &pi in &p {
            cnt += m - q.lower_bound(&(-pi));
        }
        // dbg!(x, x as f64 / B as f64, cnt, &p, &q);
        cnt < k
    });
    io.out((ans as f64 as f64 * 100.0).line());
}
