use next_permutation_impl::{next_permutation_from_i, next_permutation_from_slice};
...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    for p in next_permutation_from_i(n) {
        for q in next_permutation_from_slice(&d[..]) {
            let mut j = 0;
            let mut cur = s[p[0]].clone();
            for i in 1..n {
                cur.push('_');
                while j < q.len() && q[j] == 1 {
                    cur.push('_');
            ...
            if cur.len() >= 3 && cur.len() <= 16 && !set.contains(&cur) {
                return io.out(cur.join(""));
            }
            // dbg!(cur.join(""));
        }
