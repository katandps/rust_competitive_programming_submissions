...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (n, m, s) = io.v3::<usize, usize, usize>();
    let uvab = io.vec4::<usize, usize, usize, i64>(m);
    let mut graph = Graph::new(n);
    for (u, v, a, b) in uvab {
        graph.add_edge(u - 1, v - 1, (a, b));
    }
    let s = min!(s, 2500);
    let cd = io.vec2::<usize, i64>(n);
    let mut dp = vec![vec![1 << 60; 2501]; n];
    dp[0][s] = 0;

    // heap<Rev<t>, v, silver>
    let mut heap = BinaryHeap::new();
    heap.push((Reverse(0), 0, s));
    while let Some((Reverse(t), cur, sil)) = heap.pop() {
        if dp[cur][sil] != t {
            continue;
        }
        for (dst, (cost, dur)) in graph.edges(cur) {
            if sil < cost {
                continue;
            }
            if chmin!(dp[dst][sil - cost], t + dur) {
                heap.push((Reverse(dp[dst][sil - cost]), dst, sil - cost));
            }
        }
        let s = min!(sil + cd[cur].0, 2500);
        if chmin!(dp[cur][s], t + cd[cur].1) {
            heap.push((Reverse(dp[cur][s]), cur, s));
        }
    }
    for i in 1..n {
        let mut ans = 1 << 60;
