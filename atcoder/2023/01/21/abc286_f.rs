...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let mut v = Vec::new();
    let p = vec![4, 9, 5, 7, 11, 13, 17, 19, 23];
    for &l in &p {
        let j = v.len() as i64;
        // dbg!(j);
        for i in 1..=l {
            v.push((i % l as i64 + 1) + j);
        }
    }
    let m = v.len();
    io.out(m.ln());
    ...
    let mut k = Vec::new();
    for &pi in &p {
        // dbg!(b[i as usize] - i - 2, pi);
        k.push((b[i as usize] - i - 2).rem_euclid(pi));
        i += pi;
    }
    // dbg!(&k, &p);
    let mut ans = (0, 1);
    for i in 0..p.len() {
        ans = CRT::crt(ans.0, ans.1, k[i] as u64, p[i] as u64);
    }
    // dbg!(&b, &ans);
    io.out((ans.0 + 1).ln());
}
...
#[test]
fn t() {
    let v = vec![4, 9, 5, 7, 11, 13, 17, 19, 23];
    let mut sum = 0;
    let mut prod = 1i64;
    for vi in v {
        sum += vi;
    ...
    dbg!(sum, prod);
    assert!(sum <= 110 && prod >= 1_000_000_000);
}

#[test]
fn test() {
    let test_suits = vec!["2 3 4 1 6 7 8 9 10 11 12 13 5"];
