...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n: usize = io.v();
    let a = io.vec::<i64>(n);
    let s = io.bool_map(n, 'N');
    let q = io.v::<usize>();
    let uv = io.vec2::<usize, usize>(q);
    ...
    // dist[i][j] => (Reverse(iからjに向かう時の距離), 価値の最大)
    let mut dist = vec![vec![(Reverse(1 << 30), 0); n]; n];

    for start in 0..n {
        dist[start][start] = (Reverse(0), a[start]);

        let mut q = BinaryHeap::new();
        //
        q.push((Reverse(0), a[start], start));
        while let Some((Reverse(d), value, src)) = q.pop() {
            for (dst, _) in graph.edges(src) {
                if chmax!(dist[start][dst], (Reverse(d + 1), value + a[dst])) {
                    q.push((Reverse(d + 1), value + a[dst], dst));
                }
            }
        }
    }
    for (u, v) in uv {
        let (Reverse(dist), value) = dist[u - 1][v - 1];
        if dist == 1 << 30 {
            io.out("Impossible\n");
        } else {
            io.out(format!("{} {}\n", dist, value));
