...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let mut can_odd_change_a = false;
    let mut can_odd_change_b = false;
    for i in 2..n {
        if a[i - 2] % 2 + a[i - 1] % 2 + a[i] % 2 == 2 {
            can_odd_change_a = true;
        }
        if b[i - 2] % 2 + b[i - 1] % 2 + b[i] % 2 == 2 {
            can_odd_change_b = true;
        }
    }
    let can_even_change = even_count >= 3;
    if can_odd_change_a != can_odd_change_b {
        return io.out("No\n");
    }
    dbg!(can_even_change, can_odd_change_a);
    match (can_even_change, can_odd_change_a) {
        (true, true) => io.out("Yes\n"),
        (true, false) => {
            // 3連結以上のevenがsortできる
            // 最終的なevenの順序をそろえられたらOK
