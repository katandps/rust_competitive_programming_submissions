...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let mut ans = 0;
    dfs(0, &mut HashSet::default(), &mut ans, &mut &graph);
    io.out(min!(ans, 1000000).ln());
}

fn dfs(cur: usize, done: &mut HashSet<usize>, ans: &mut usize, graph: &Graph<usize>) {
    if *ans >= 1000000 {
        return;
    }
    *ans += 1;
    done.insert(cur);
    for (dst, _) in graph.edges(cur) {
        if done.contains(&dst) {
            continue;
        }
        dfs(dst, done, ans, graph);
    }
    done.remove(&cur);
}
