...
impl<K: Copy + Hash + Eq + Default, T: Default> Trie<K, T> {
    ...
    pub fn same(&self, key: &[K]) -> usize {
        match key.first() {
            Some(c) => self
                .childs
                .get(c)
                .and_then(|node| Some(node.same(&key[1..]) + 1))
                .unwrap_or(0),
            None => 0,
        }
    }
...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n: usize = io.v();
    let s = (0..n).map(|_| io.chars()).collect::<Vec<_>>();
    let mut left_trie = Trie::default();
    let mut ans = vec![0; n];
    for i in 0..n {
        chmax!(ans[i], left_trie.same(&s[i]));
        left_trie.insert(&s[i], 1);
    }
    let mut right_trie = Trie::default();
    for i in (0..n).rev() {
        chmax!(ans[i], right_trie.same(&s[i]));
        right_trie.insert(&s[i], 1);
    }
    io.out(ans.join("\n"));
}
#[test]
