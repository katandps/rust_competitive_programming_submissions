...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (n, t) = io.v2::<usize, usize>();
    let ab = io.vec2::<usize, usize>(n);
    let mut dp = vec![vec![0; 2]; t];
    for i in 0..n {
        let (a, b) = ab[i];
        // 最後に食べることにする
        ...
        if t > a {
            for j in (0..t - a).rev() {
                chmax!(dp[j + a][1], dp[j][1] + b);
            }
        }
        for j in (0..t).rev() {
            chmax!(dp[j][1], dp[j][0] + b);
        }
        if t > a {
            for j in (0..t - a).rev() {
                chmax!(dp[j + a][0], dp[j][0] + b);
            }
        }
    }
    let mut ans = 0;
    for i in 0..t {
        chmax!(ans, dp[i][0], dp[i][1]);
    }
    io.out(ans.ln());
}
