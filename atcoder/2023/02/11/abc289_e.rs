...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    for _ in 0..n {
        ...
        q.push((Reverse(0), (0, n - 1)));
        while let Some((Reverse(d), (takahashi, aoki))) = q.pop() {
            if dp[takahashi][aoki] != d {
                continue;
            }
            for (t_dst, _) in graph.edges(takahashi) {
                for (a_dst, _) in graph.edges(aoki) {
                    if c[t_dst] == c[a_dst] {
                        continue;
                    }
                    if chmin!(dp[t_dst][a_dst], dp[takahashi][aoki] + 1) {
                        q.push((Reverse(dp[t_dst][a_dst]), (t_dst, a_dst)));
                    }
                }
            }
        }
