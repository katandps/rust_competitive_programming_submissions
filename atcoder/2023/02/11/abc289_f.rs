...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (sx, sy, tx, ty) = io.v4::<i64, i64, i64, i64>();
    let (a, b, c, d) = io.v4::<i64, i64, i64, i64>();
    // 操作なし
    if sx == tx && sy == ty {
        return io.out("Yes\n");
    }
    ...
    if (sx - tx) % 2 == 0 && (sy - ty) % 2 == 0 {
        let (mut xd, mut yd) = ((tx - sx) / 2, (ty - sy) / 2);
        // a..=bを10^6回以下使ってsdを作る -と+を交互に使う
        // sd<=2*10^5なので、a==bでなければ可能
        let mut ng = false;
        if xd.abs() > 0 && a == b {
            ng = true;
        }
        ...
        let mut xans = Vec::new();
        let mut yans = Vec::new();
        dbg!(sx, sy);
        if ng {
            let sx = 2 * a - sx;
            let sy = 2 * c - sy;
