...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n = io.v::<usize>();
    let uv = io.vec2::<usize, usize>(n - 1);
    if n == 2 {
        return io.out("2 1\n");
    }
    ...
    // for i in 0..n {
    //     dfs(Vec::new(), i, i, &graph, &mut paths);
    // }
    // io.out(1);
    // dbg!(&paths);
}
...
fn dfs(
    ...
    for (dst, _) in graph.edges(src) {
        ...
        dfs(cur.clone(), dst, src, graph, paths);
    }
    cur.pop();
}
