...
// codesnip-guard: next-permutation
#[cfg_attr(any(),rustfmt::skip)]pub use self::next_permutation_impl::{next_permutation_from_i,next_permutation_from_slice};
#[cfg_attr(any(),rustfmt::skip)]mod next_permutation_impl{#[doc=" [0, size)の順列を辞書順に得るIteratorとなる"]pub fn next_permutation_from_i(size:usize)->impl Iterator<Item=Vec<usize>>{std::iter::successors(Some((0..size).collect::<Vec<_>>()),move|v|{let mut v=v.to_vec();if v.len()>=2{for i in(1..v.len()).rev(){if v[i-1]<v[i]{let j=(0..v.len()).rev().find(|j|v[*j]>v[i-1]).unwrap();v.swap(i-1,j);v[i..].reverse();return Some(v.to_vec());}}}None})}#[doc=" 重複を考慮した順列を辞書順に得るIteratorとなる"]pub fn next_permutation_from_slice<T:Clone+PartialOrd>(src:&[T])->impl Iterator<Item=Vec<T>>{std::iter::successors(Some(src.to_vec()),move|v|{let mut v=v.to_vec();if v.len()>=2{for i in(1..v.len()).rev(){if v[i-1]<v[i]{let j=(0..v.len()).rev().find(|j|v[*j]>v[i-1]).unwrap();v.swap(i-1,j);v[i..].reverse();return Some(v.to_vec());}}}None})}}

// codesnip-guard: bit-set
#[cfg_attr(any(),rustfmt::skip)]pub use bitset_impl::BitSet;
#[cfg_attr(any(),rustfmt::skip)]mod bitset_impl{use super::{BitAnd,BitOr,BitXor,Debug,Display,Formatter,FromIterator,Index,Not,Shl,ShlAssign,Shr,ShrAssign};#[derive(Clone,Eq,PartialEq)]pub struct BitSet{bits:Vec<u64>,size:usize}impl BitSet{#[doc=" bitの長さ $2^6 = 64$"]const BLOCK_LEN:usize=u64::BITS as usize;#[doc=" # bitの長さの長さ"]#[doc=" indexからu64単位の場所を割り出すのに使う"]const BLOCK_LEN_LEN:usize=Self::BLOCK_LEN.trailing_zeros()as usize;pub fn new(size:usize)->Self{let bits=vec![0;(size+Self::BLOCK_LEN-1)/Self::BLOCK_LEN];Self{bits,size}}#[doc=" indexビット目のビットをbに変える"]pub fn set(&mut self,index:usize,b:bool){assert!(index<self.size);if b{self.bits[index>>Self::BLOCK_LEN_LEN]|=1<<(index&(Self::BLOCK_LEN-1));}else{self.bits[index>>Self::BLOCK_LEN_LEN]&=!(1<<(index&(Self::BLOCK_LEN-1)));}}#[doc=" indexビット目のbitを反転する"]pub fn rev(&mut self,index:usize){assert!(index<self.size);self.bits[index>>Self::BLOCK_LEN_LEN]^=1<<(index&(Self::BLOCK_LEN-1));}#[doc=" 立っているビットの数を数える"]pub fn count_ones(&self)->u32{self.bits.iter().map(|b|b.count_ones()).sum()}#[doc=" 下位64bitをu64として取り出す"]pub fn get_u64(&self)->u64{self.bits[0]}fn chomp(&mut self){let r=self.size&(Self::BLOCK_LEN-1);if r!=0{let d=Self::BLOCK_LEN-r;if let Some(x)=self.bits.last_mut(){*x=(*x<<d)>>d;}}}}impl Debug for BitSet{fn fmt(&self,f:&mut Formatter)->std::fmt::Result{Display::fmt(self,f)}}impl Index<usize>for BitSet{type Output=bool;fn index(&self,index:usize)->&bool{assert!(index<self.size);&[false,true][((self.bits[index>>Self::BLOCK_LEN_LEN]>>(index&(Self::BLOCK_LEN-1)))&1)as usize]}}impl BitAnd for BitSet{type Output=BitSet;fn bitand(self,rhs:Self)->Self::Output{&self& &rhs}}impl BitAnd for&BitSet{type Output=BitSet;fn bitand(self,rhs:Self)->Self::Output{assert_eq!(self.size,rhs.size);BitSet{bits:self.bits.iter().zip(rhs.bits.iter()).map(|(l,r)|l&r).collect(),size:self.size}}}impl BitOr for BitSet{type Output=Self;fn bitor(self,rhs:Self)->Self::Output{assert_eq!(self.size,rhs.size);Self{bits:(0..self.bits.len()).map(|i|self.bits[i]|rhs.bits[i]).collect(),size:self.size}}}impl BitXor for BitSet{type Output=Self;fn bitxor(self,rhs:Self)->Self::Output{assert_eq!(self.size,rhs.size);Self{bits:(0..self.bits.len()).map(|i|self.bits[i]^rhs.bits[i]).collect(),size:self.size}}}impl ShlAssign<usize>for BitSet{fn shl_assign(&mut self,rhs:usize){if rhs>=self.size{self.bits.iter_mut().for_each(|b|*b=0);return;}let block=rhs>>Self::BLOCK_LEN_LEN;let inner=rhs&(Self::BLOCK_LEN-1);if inner==0{(block..self.bits.len()).rev().for_each(|i|self.bits[i]=self.bits[i-block])}else{(block+1..self.bits.len()).rev().for_each(|i|{self.bits[i]=(self.bits[i-block]<<inner)|(self.bits[i-block-1]>>(Self::BLOCK_LEN-inner))});self.bits[block]=self.bits[0]<<inner;}self.bits[..block].iter_mut().for_each(|b|*b=0);self.chomp();}}impl Shl<usize>for BitSet{type Output=Self;fn shl(mut self,rhs:usize)->Self::Output{self<<=rhs;self}}impl ShrAssign<usize>for BitSet{fn shr_assign(&mut self,rhs:usize){if rhs>=self.size{self.bits.iter_mut().for_each(|b|*b=0);return;}let block=rhs>>Self::BLOCK_LEN_LEN;let inner=rhs&(Self::BLOCK_LEN-1);let len=self.bits.len();if inner==0{(0..len-block).for_each(|i|self.bits[i]=self.bits[i+block])}else{(0..len-block-1).for_each(|i|{self.bits[i]=(self.bits[i+block]>>inner)|(self.bits[i+block+1]<<(Self::BLOCK_LEN-inner))});self.bits[len-block-1]=self.bits[len-1]>>inner;}self.bits[len-block..].iter_mut().for_each(|b|*b=0);}}impl Shr<usize>for BitSet{type Output=Self;fn shr(mut self,rhs:usize)->Self::Output{self>>=rhs;self}}impl Not for BitSet{type Output=Self;fn not(self)->Self::Output{Self{bits:self.bits.iter().map(|&i|i^u64::MAX).collect(),size:self.size}}}impl Display for BitSet{fn fmt(&self,f:&mut Formatter<'_>)->std::fmt::Result{write!(f,"{}",(0..self.size).rev().map(|i|usize::from(self[i]).to_string()).collect::<String>())}}impl<Item:Into<bool>>FromIterator<Item>for BitSet{fn from_iter<T:IntoIterator<Item=Item>>(iter:T)->Self{let mut bits=Vec::new();let mut size=0;let mut cur=0;for item in iter.into_iter(){if item.into(){cur+=1<<(size%64);}size+=1;if size%u64::BITS as usize==0{bits.push(cur);cur=0;}}if size%u64::BITS as usize!=0{bits.push(cur)}Self{bits,size}}}}
// codesnip-guard: zzz-solver
pub fn solve(read: impl Read, mut write: impl Write) {
    let mut reader = ReadHelper::new(read);
    let (n, h, w) = reader.v3::<usize, usize, usize>();
    let ab = reader.vec2::<usize, usize>(n);
    for p in next_permutation_from_i(n) {
        // dbg!(&p);
        let mut q = Vec::new();
        let empty = BitSet::new(h * w);
        q.push((0, empty));
        while let Some((i, mut bitset)) = q.pop() {
            let mut all = true;
            for i in 0..h {
                for j in 0..w {
                    if !bitset[i * w + j] {
            ...
            if all {
                return writeln!(write, "Yes").unwrap();
            }
            // dbg!(i, &spot);
            // println!(
            //     "{}",
            //     &map.iter()
            //         .map(|v| v.iter().map(|b| if *b { "T" } else { "F" }).join(""))
            //         .join("\n"),
            // );
            if i == n {
                continue;
            }
            q.push((i + 1, bitset.clone()));
            let (next_h, next_w) = ab[p[i]];
            let mut done = false;
            for y in 0..h {
                if done {
                    break;
                ...
                for x in 0..w {
                    if !bitset[y * w + x] {
                        let mut bitset_next = bitset.clone();
                        let mut ng = false;
                        if y + next_h > h || x + next_w > w {
                            ng = true;
                        }
                        for y in y..y + next_h {
                            if ng {
                            ...
                            for x in x..x + next_w {
                                if bitset_next[y * w + x] {
                                    ng = true;
                                    break;
                                }
                                bitset_next.set(y * w + x, true);
                            }
                        }

                        if !ng {
                            q.push((i + 1, bitset_next))
                        }
                        let mut ng = false;
                        if y + next_w > h || x + next_h > w {
                            ng = true;
                        }
                        for y in y..y + next_w {
                            if ng {
