...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for i in 0..n {
        forest.entry(uf.root(i)).or_insert(Vec::new()).push(i);
    }
    // dbg!(&forest);
    let mut hole = Vec::new();
    let mut lonely = Vec::new();
    let mut ans = Vec::new();
    for (_root, nodes) in forest {
        let mut dsum = 0;
        let mut buf = Vec::new();
        for &v in &nodes {
            dsum += d[v];
        ...
        if let Some(v) = buf.pop() {
            if let Some(w) = hole.pop() {
                ans.push(vec![v + 1, w + 1]);
            } else {
                hole.push(v);
            }
        } else {
    ...
    if hole.is_empty() && lonely.len() == 2 {
        ans.push(vec![lonely[0] + 1, lonely[1] + 1]);
    } else if hole.len() != lonely.len() {
        return writer.ln(-1);
    }
    for i in 0..hole.len() {
        ans.push(vec![hole[i] + 1, lonely[i] + 1]);
    }
    for a in ans {
        writer.join(&a, " ");
    }
