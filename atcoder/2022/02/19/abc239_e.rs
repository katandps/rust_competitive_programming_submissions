...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let vk = reader.vec2::<usize, usize>(q);
    ...
    dfs(0, 0, &mut ans, &tree, &x);
    for (v, k) in vk {
        writer.ln(ans[v - 1][k - 1]);
    }
}

fn dfs(cur: usize, par: usize, ans: &mut Vec<Vec<i64>>, g: &Graph<()>, x: &Vec<i64>) {
    let mut buf = vec![x[cur]];
