...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    a_sort.dedup();
    if a_sort.len() < n {
        return writer.ln("Yes");
    }

    let mut bita: BinaryIndexedTree<Addition<i64>> = BinaryIndexedTree::from(5001);
    let mut reta = 0;
    for i in 0..n {
        let ai = a[i];
        reta += bita.fold(0..ai);
    ...
    let mut bitb: BinaryIndexedTree<Addition<i64>> = BinaryIndexedTree::from(5001);
    let mut retb = 0;
    for i in 0..n {
        let bi = b[i];
        retb += bitb.fold(0..bi);
    ...
    writer.ln(if (reta - retb) % 2 == 0 { "Yes" } else { "No" });
    // dbg!(reta, retb);
}
