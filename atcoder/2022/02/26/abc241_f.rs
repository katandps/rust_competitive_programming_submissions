...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (h, w, n) = reader.v3::<usize, usize, usize>();
    let (sx, sy) = reader.v2::<usize, usize>();
    let (gx, gy) = reader.v2::<usize, usize>();
    let xy = reader.vec2::<usize, usize>(n);

    let mut points = HashMap::new();
    let mut points_rev = HashMap::new();
    let mut rows = HashMap::new();
    let mut columns = HashMap::new();

    points.insert((sx, sy), 0);
    points_rev.insert(0, (sx, sy));
    let mut cur = 1;
    for &(x, y) in &xy {
        rows.entry(y).or_insert(vec![0, h]).push(x);
        columns.entry(x).or_insert(vec![0, w]).push(y);

        let ps = vec![(x - 1, y), (x, y - 1), (x + 1, y), (x, y + 1)];
        for (x, y) in ps {
            if points.contains_key(&(x, y)) {
                continue;
            }
            points.insert((x, y), cur);
            points_rev.insert(cur, (x, y));
            cur += 1;
        }
    }
    if !points.contains_key(&(gx, gy)) {
        return writer.ln(-1);
    }
    ...
    for (&y, row) in rows.iter_mut() {
        row.sort();
        for i in 1..row.len() {
            if let Some(p) = points.get(&(row[i - 1] + 1, y)) {
                if let Some(q) = points.get(&(row[i] - 1, y)) {
                    graph.add_edge(*p, *q, 1);
                }
            }
        }
    }
    for (&x, column) in columns.iter_mut() {
        column.sort();
        for i in 1..column.len() {
            if let Some(p) = points.get(&(x, column[i - 1] + 1)) {
                if let Some(q) = points.get(&(x, column[i] - 1)) {
                    graph.add_edge(*p, *q, 1);
                }
            }
        }
    ...
    for i in 0..graph.size() {
        let &(x, y) = points_rev.get(&i).unwrap();
        // dbg!(x, y);
        if let Some(row) = rows.get_mut(&y) {
            // dbg!(&row);
            for j in 1..row.len() {
                if row[j - 1] <= x && x <= row[j] {
                    if let Some(p) = points.get(&(row[j - 1] + 1, y)) {
                        graph.add_arc(i, *p, 1);
                    }
                    if let Some(p) = points.get(&(row[j] - 1, y)) {
                        graph.add_arc(i, *p, 1);
                    }
                }
            }
        }
        if let Some(column) = columns.get_mut(&x) {
            // dbg!(&column);
            for j in 1..column.len() {
                if column[j - 1] <= y && y <= column[j] {
                    if let Some(p) = points.get(&(x, column[j - 1] + 1)) {
                        graph.add_arc(i, *p, 1);
                    }
                    if let Some(p) = points.get(&(x, column[j] - 1)) {
                        graph.add_arc(i, *p, 1);
                    }
                }
            }
    ...
    if let Some(&g) = points.get(&(gx, gy)) {
        let d = Dijkstra::calc(&graph, 0);
        // dbg!(&graph, &d.dist, &points_rev);
        if d.dist[g] == std::i32::MAX {
            writer.ln(-1);
        } else {
            writer.ln(d.dist[g]);
        }
