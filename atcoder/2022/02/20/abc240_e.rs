...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let uv = reader.vec2::<usize, usize>(n - 1);
    ...
    dfs_width(0, 0, &tree, &mut width);
    let mut margin = vec![0; n];
    dfs_margin(0, 0, 0, &tree, &width, &mut margin);
    let mut ans = Vec::new();
    for i in 0..n {
        ans.push(vec![1 + margin[i], width[i] + margin[i]]);
    }
    for a in ans {
        writer.join(&a, " ");
    }
...
fn dfs_width(cur: usize, par: usize, tree: &Graph<()>, width: &mut Vec<usize>) -> usize {
    let mut childs_sum = 0;
    for (dst, ()) in tree.edges(cur) {
        if dst == par {
            continue;
        }
        childs_sum += dfs_width(dst, cur, tree, width);
    }
    width[cur] = if childs_sum == 0 { 1 } else { childs_sum };
    width[cur]
}
...
fn dfs_margin(
    ...
    for (dst, ()) in tree.edges(cur) {
        ...
        dfs_margin(dst, cur, mar, tree, width, margin);
        mar += width[dst];
    }
}
