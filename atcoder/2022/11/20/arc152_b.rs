...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    // 同じ休憩所からスタート → L歩いたところから一番近い場所ですれ違うのが最善
    // ほかの場合は、この場合に帰着される
    let mut set = BTreeSet::new();
    for i in 0..n {
        set.insert(a[i]);
        set.insert(l * 2 - a[n - 1 - i]);
    }
    dbg!(&set);
    let mut ans = 1 << 60;
    for i in 0..n {
        if let Some(left) = set.range(..=a[i] + l).last() {
            chmin!(ans, 2 * (l * 2 - (left - a[i])));
        }
        if let Some(right) = set.range(a[i] + l..).next() {
            chmin!(ans, 2 * (right - a[i]));
        }
    }
    io.out(ans.ln());
}
