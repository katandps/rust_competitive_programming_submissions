...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for i in 0..n {
        let (ai, i, m) = (a[i], i as i64 + 1, m as i64);
        // 初めて0以上になる回数
        let left = binary_search(m + 1, 0, |mid| ai + mid * i >= 0);
        // 初めてn以上になる回数
        let right = binary_search(m + 1, 0, |mid| ai + mid * i >= n as i64);
        dbg!(i, ai, left, right);
        for j in left..right {
            p[j as usize].push(ai + j * i);
        }
    }
    'case: for i in 1..=m {
        p[i].sort();
