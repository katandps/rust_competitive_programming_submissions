...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    const COL_MAX: usize = 100000;
    const ROW_MAX: usize = 100000;
    let mut cols = vec![Vec::new(); COL_MAX + 1];
    let mut rows = vec![Vec::new(); ROW_MAX + 1];
    for &(x, y) in &xy {
        cols[x].push(y);
        rows[y].push(x);
    }
    let mut row_uf = UnionFind::new(ROW_MAX + 1);
    for c in &cols {
        for y in c.windows(2) {
            row_uf.unite(y[0], y[1]);
        }
    }
    let mut col_uf = UnionFind::new(COL_MAX + 1);
    for r in &rows {
        for x in r.windows(2) {
            col_uf.unite(x[0], x[1]);
        }
    }
    let mut ans = 0;
    for y in 1..=ROW_MAX {
        if let Some(&x) = rows[y].get(0) {
            dbg!(row_uf.size(y), col_uf.size(x), y, x,);
            if y != row_uf.root(y) {
                continue;
            }
            ans += row_uf.size(y) * col_uf.size(x);
        }
    }
    writer.ln(ans - n);
    // writer.join(
    //     &(1..=COL_MAX).map(|i| col_uf.root(i)).collect::<Vec<_>>(),
    //     " ",
    // );
    // writer.join(
    //     &(1..=ROW_MAX).map(|i| row_uf.root(i)).collect::<Vec<_>>(),
    //     " ",
    // );
}
