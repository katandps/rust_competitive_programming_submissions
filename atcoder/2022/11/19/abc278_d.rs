...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let q: usize = io.v();
    // (時刻、 プラス値)
    let mut sum = vec![vec![(0, 0)]; n + 1];
    let mut reset_time = 1;
    let mut reset_val = 0;
    for i in 0..n {
        sum[i + 1].push((i + 1, a[i]));
    }
    for t in n + 1..n + 1 + q {
        match io.v::<usize>() {
            1 => {
            ...
            2 => {
                let (i, x) = io.v2::<usize, usize>();
                let (_, last_val) = sum[i][sum[i].len() - 1];
                sum[i].push((t, last_val + x));
            }
            3 => {
                let i = io.v::<usize>();
                let s = binary_search(0, sum[i].len() as i64, |m| {
                    sum[i][m as usize].0 < reset_time
                }) as usize;
                dbg!(&sum[i]);
                io.out((reset_val + sum[i][sum[i].len() - 1].1 - sum[i][s].1).ln())
            }
            _ => unreachable!(),
        }
    }
