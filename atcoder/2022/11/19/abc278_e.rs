...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (h, w, n, sh, sw) = io.v5::<usize, usize, usize, usize, usize>();
    let a = io.matrix::<usize>(h, w);
    let mut colors = vec![Vec::new(); n + 1];
    for y in 0..h {
        for x in 0..w {
            colors[a[y][x]].push((y, x));
        }
    }

    let mut ans = vec![vec![0; w - sw + 1]; h - sh + 1];
    for v in colors {
        if v.is_empty() {
            continue;
        }
        let (mut max_x, mut min_x, mut max_y, mut min_y) = (v[0].1, v[0].1, v[0].0, v[0].0);
        for i in 1..v.len() {
            chmax!(max_x, v[i].1);
            chmax!(max_y, v[i].0);
            chmin!(min_x, v[i].1);
        ...
        // dbg!(max_y, min_y, max_x, min_x);
        for y in 0..h - sh + 1 {
            for x in 0..w - sw + 1 {
                // 範囲の最大値
                let (ly, lx) = (y + sh - 1, x + sw - 1);
                // dbg!(ly,y,lx,x);
                if min_y < y || min_x < x || max_y > ly || max_x > lx {
                    ans[y][x] += 1;
                }
            }
        }
