...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let q: usize = io.v();
    // ボールの合流状況を表す
    let mut uf = UnionFind::new(n + q + 1);
    // ある箱に入っている代表的なボール 0 -> ボールなし
    let mut spec = (0..=n).collect::<Vec<_>>();
    // ルートボールが入っているボックスの番号
    let mut root = (0..=q + n).collect::<Vec<_>>();
    let mut next = n + 1;
    for _ in 0..q {
        //debug
        // dbg!((0..=n + q).map(|i| uf.root(i)).join(" "));
        match io.v::<usize>() {
            1 => {
                let (x, y) = io.v2::<usize, usize>();
                // yが空だと何もしない
                if spec[y] != 0 {
                    // xが空でない → xとyを合流
                    if spec[x] != 0 {
                        uf.unite(spec[x], spec[y]);
                        spec[x] = uf.root(spec[x]);
                        root[uf.root(spec[x])] = x;
                    } else {
                        // xが空 → xをyの情報で上書き
                        spec[x] = uf.root(spec[y]);
                        root[uf.root(spec[x])] = x;
                    }
                    // yは空にする
                    spec[y] = 0;
