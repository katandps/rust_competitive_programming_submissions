...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n: usize = io.v();
    let m: usize = io.v();
    let a = io.vec::<usize>(m);
    // 1がいる場所 → 最初はどのパターンも1にいる
    let mut one_pos = vec![HashSet::default(); n];
    for i in 0..m {
        one_pos[0].insert(i);
    }
    for i in 0..m {
        ...
        // ai番目にいる→+1に移動
        // ai+1番目にいる→-1に移動
        // ただし、one_pos内のiだけは移動しない
        one_pos.swap(ai, ai + 1);
        if one_pos[ai].contains(&i) {
            one_pos[ai + 1].insert(i);
            one_pos[ai].remove(&i);
        } else if one_pos[ai + 1].contains(&i) {
            one_pos[ai].insert(i);
            one_pos[ai + 1].remove(&i);
        }
    }
    let mut ans = vec![0; m];
    for i in 0..n {
