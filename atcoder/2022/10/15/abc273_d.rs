...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (h, w) = reader.v2::<i64, i64>();
    let (mut rs, mut cs) = reader.v2::<i64, i64>();
    let n = reader.v::<usize>();
    let rc = reader.vec2::<i64, i64>(n);
    let q = reader.v::<usize>();
    let dl = reader.vec2::<char, i64>(q);
    let mut wall_r = HashMap::new();
    let mut wall_c = HashMap::new();
    for (r, c) in rc {
        wall_r.entry(r).or_insert(Vec::new()).push(c);
        wall_c.entry(c).or_insert(Vec::new()).push(r);
    }
    for (_k, v) in wall_r.iter_mut() {
        v.sort();
    }
    for (_k, v) in wall_c.iter_mut() {
        v.sort();
    }
    for (d, l) in dl {
        if rs < 1 || rs > h || cs < 1 || cs > w {
            return;
        }
        match d {
            'L' => {
                if let Some(v) = wall_r.get(&rs) {
                    let block = v.lower_bound(&cs);
                    if block > 0 {
                        cs = max!(v[block - 1] + 1, cs - l);
                    } else {
                        cs = max!(1, cs - l);
                    }
                } else {
            ...
            'R' => {
                if let Some(v) = wall_r.get(&rs) {
                    let block = v.upper_bound(&cs);
                    if let Some(b) = v.get(block) {
                        cs = min!(cs + l, *b - 1);
                    } else {
                        cs = min!(cs + l, w);
                    }
            ...
            'U' => {
                if let Some(v) = wall_c.get(&cs) {
                    let block = v.lower_bound(&rs);
                    if block > 0 {
                        rs = max!(v[block - 1] + 1, rs - l);
                    } else {
                        rs = max!(1, rs - l);
                    }
                } else {
