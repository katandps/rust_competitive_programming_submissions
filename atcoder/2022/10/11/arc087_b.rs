...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let x_sum = v[1].iter().skip(1).sum::<usize>();
    if x_sum < (x - v[1][0] as i64).abs() as usize
        || (x_sum - (x - v[1][0] as i64).abs() as usize) % 2 != 0
    {
        return writer.ln("No");
    }
    let target_x = (x_sum - (x - v[1][0] as i64).abs() as usize) / 2;
    let mut dp_x = vec![false; target_x + 1];
    dp_x[0] = true;
    for i in 1..v[1].len() {
        let d = v[1][i];
        for dst in (d..=target_x).rev() {
            dp_x[dst] |= dp_x[dst - d];
        }
    }
    let y_sum = v[0].iter().sum::<usize>();
    if y_sum < y.abs() as usize || (y_sum - y.abs() as usize) % 2 != 0 {
        return writer.ln("No");
    }
    let target_y = (y_sum - y.abs() as usize) / 2;
    let mut dp_y = vec![false; target_y + 1];
    dp_y[0] = true;
    for i in 0..v[0].len() {
        let d = v[0][i];
        for dst in (d..=target_y).rev() {
            dp_y[dst] |= dp_y[dst - d];
        }
    }
    dbg!(&dp_x, &dp_y, v);
    writer.ln(if dp_x[target_x] && dp_y[target_y] {
        "Yes"
    } else {
        "No"
    });
