...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (r, g, b) = reader.v3::<i64, i64, i64>();
    // dp[i][j] = i個のマーブルを左にi-500個動かす
    let mut dp = vec![vec![0; 1000]; 301];
    for i in 1..=300 {
        for j in 0..1000 {
            // 左に l = 500-j マスずれたところからスタート
            let l = if j < 500 { 500 - j } else { 0 };
            let mut rest = i;
            if l > 0 {
                if l >= rest {
                    dp[i][j] = (l + l - rest + 1) * rest / 2;
                    rest = 0;
                } else {
                    dp[i][j] = l * (l + 1) / 2;
                    rest -= l;
            ...
            let r = if j > 500 { j - 500 } else { 0 };
            if rest > 0 {
                dp[i][j] += (rest - 1 + r * 2) * rest / 2;
            }
        }
    }
    dbg!(dp[3][496], dp[3][497], dp[3][498], dp[3][499], dp[3][500], dp[3][501], dp[3][502]);
    let mut ans = 1 << 30;
    // gの左端をg_leftとする
    for g_left in -300..=300 {
        let mut k = dp[g as usize][(500 + g_left) as usize];
        let mut r_min = 1 << 30;
        for r_right in -300..g_left {
            chmin!(r_min, dp[r as usize][(500 - (r_right + 100)) as usize]);
        }
        k += r_min;
        let mut b_min = 1 << 30;
        for b_left in g_left + g..300 {
            chmin!(b_min, dp[b as usize][(500 - 100 + b_left) as usize]);
        }
        k += b_min;
        if chmin!(ans, k) {
            dbg!(g_left, k);
