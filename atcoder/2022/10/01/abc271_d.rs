...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, s) = reader.v2::<usize, usize>();
    let ab = reader.vec2::<usize, usize>(n);
    // (置けるかどうか、最後に置いたカードがAか, 置いた値)
    let mut dp = vec![vec![(false, false, 0); s + 1]; n + 1];
    dp[0][0] = (true, false, 0);
    for i in 0..n {
        let (a, b) = ab[i];
        for src in 0..s {
            ...
            if src + a <= s {
                dp[i + 1][src + a] = (true, true, a);
            }
            if src + b <= s {
                dp[i + 1][src + b] = (true, false, b);
            }
        }
    }
    if !dp[n][s].0 {
    ...
    let mut ans = Vec::new();
    for i in (1..=n).rev() {
        ans.push(if dp[i][cur].1 { "H" } else { "T" });
        cur -= dp[i][cur].2;
    }
    ans.reverse();
    writer.ln("Yes");
