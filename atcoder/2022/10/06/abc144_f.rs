...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let st = reader.vec2::<usize, usize>(m);
    let mut graph = Graph::new(n);
    for i in 0..m {
        let (s, t) = st[i];
        graph.add_arc(s - 1, t - 1, i);
    }
    let mut to_sum = vec![n as f64; n];
    let mut to_max = vec![0.0; n];
    let mut dp = vec![0.0; n];
    let deg = graph.outdegree();
    for j in (0..n - 1).rev() {
        let mut sum = 0.0;
        for (dst, _) in graph.edges(j) {
            to_sum[j] += dp[dst];
            chmax!(to_max[j], dp[dst]);
            sum += dp[dst];
        }
        dp[j] = 1.0 + sum / deg[j] as f64;
    }
    ...
    for remove in 0..n - 1 {
        ...
        for j in (0..=remove).rev() {
            ...
            if j == remove {
                dp[j] = 1.0 + (sum - max) / (deg[j] - 1) as f64;
            } else {
                dp[j] = 1.0 + sum / deg[j] as f64;
            }
        }
