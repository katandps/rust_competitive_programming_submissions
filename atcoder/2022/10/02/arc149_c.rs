...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    if n == 3 {
        ...
        let mut next = 0;
        while next < n {
            let e = evens.pop_front().unwrap();
            let o = odds.pop_front().unwrap();
            if (e + o).is_prime() {
                evens.push_back(e);
                odds.push_front(o);
            } else {
        ...
        for i in 0..n / 2 - 1 {
            for j in 0..n {
                ans[i][j] = evens.pop_front().unwrap();
            }
        }
        for i in n / 2 + 1..n {
            for j in 0..n {
                ans[i][j] = odds.pop_front().unwrap();
            }
        }
    } else {
        for i in 0.. {
            if let Some(o) = odds.pop_front() {
                ans[i / n][i % n] = o;
            } else {
                break;
            }
        ...
        let k = n * n - evens.len();
        while let Some(e) = evens.pop_front() {
            if (e + ans[(k - 1) / n][(k - 1) % n]).is_prime()
                || (e + ans[(k - n) / n][(k - n) % n]).is_prime()
            {
                evens.push_back(e);
            } else {
                ans[k / n][k % n] = e;
        ...
        for i in n * n - evens.len()..n * n {
            let (r, c) = (i / n, i % n);
            while let Some(e) = evens.pop_front() {
                if (e + ans[r - 1][c]).is_prime() {
                    evens.push_back(e);
                } else {
                    ans[r][c] = e;
                    break;
