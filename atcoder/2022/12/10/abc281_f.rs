...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    io.out(f(30, &a, &mut map).ln());
    // dbg!(&map);
}

fn f(b: usize, v: &[u64], memo: &mut HashMap<(usize, Vec<u64>), u64>) -> u64 {
    let v = v.to_vec();
    if let Some(r) = memo.get(&(b, v.clone())) {
        return *r;
    }
    if v.is_empty() {
        return 0;
    }
    let mut vv = vec![Vec::new(), Vec::new()];
    for i in 0..v.len() {
        vv[usize::from((v[i] & 1 << b) > 0)].push((v[i] | 1 << b) - (1 << b));
    }
    let k = if vv[0].is_empty() || vv[1].is_empty() {
        0
    } else {
        1 << b
    };
    ...
    let (s, t) = if b > 0 {
        (f(b - 1, &vv[0], memo), f(b - 1, &vv[1], memo))
    } else {
        (0, 0)
    };
    let ret = min!(max!(s + k, t), max!(s, t + k));
    memo.insert((b, v), ret);
    return ret;
}
