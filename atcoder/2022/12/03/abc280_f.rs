...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let xy = io.vec2::<usize, usize>(q);

    let mut uf = WeightedUnionFind::<Addition<i64>>::new(n + 1);
    let mut inf_nodes = Vec::new();
    for (a, b, c) in abc {
        if uf.same(a, b) {
            let w = uf.diff(a, b);
            if w != c {
    ...
    for (x, y) in xy {
        if uf.same(x, y) {
            if inf_roots.contains(&uf.root(x)) {
                io.out("inf\n");
            } else {
                io.out(uf.diff(x, y).ln());
            }
        } else {
            io.out("nan\n");
        }
