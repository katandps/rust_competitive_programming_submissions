...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (mut a, mut b) = io.v2::<usize, usize>();
    a -= 1;
    b -= 1;
    let (h, w) = (70, 70);
    ...
    for i in (0..h / 4).map(|i| i * 2) {
        for j in (0..w / 2).map(|j| j * 2 + 2).take_while(|v| *v < w - 1) {
            if a == 0 {
                break;
            }
            a -= 1;
    ...
    for i in (h / 4..h / 2).map(|i| i * 2 + 1) {
        for j in (0..w / 2).map(|j| j * 2 + 1).take_while(|v| *v < w - 1) {
            if b == 0 {
                break;
            }
            b -= 1;
    ...
    io.out(format!("{} {}\n", h, w));
    for a in ans {
        io.out(a.iter().map(|b| if *b { '.' } else { '#' }).join(""));
    }
}

#[test]
fn test() {
    let test_suits = vec!["2 3", "7 8", "3 14", "500 500"];
