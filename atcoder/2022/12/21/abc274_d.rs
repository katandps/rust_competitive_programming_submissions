...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let n: usize = io.v();
    let (x, y) = io.v2::<i64, i64>();
    let a = io.vec::<usize>(n);
    let mut s = vec![Vec::new(), Vec::new()];
    let x = x - a[0] as i64;
    for i in 1..n {
        s[i % 2].push(a[i]);
    }
    if knapsack(x.abs() as usize, &s[0]) && knapsack(y.abs() as usize, &s[1]) {
        io.out("Yes\n")
    } else {
        io.out("No\n")
    }
...
fn knapsack(total: usize, src: &[usize]) -> bool {
    let mut dp = vec![false; 20010];
    dp[10000] = true;
    for s in src {
        let mut next = vec![false; 20010];
        for i in 0..=20000 {
            if i + s <= 20000 {
                next[i + s] |= dp[i];
            }
