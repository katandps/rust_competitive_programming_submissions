...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    let pq = io.vec2::<f64, f64>(m);
    // dp[宝の取得状況][ブースターの取得状況][現在地]
    let mut dp = vec![vec![vec![1e100; n + m + 1]; 1 << m]; 1 << n];
    dp[0][0][0] = 0.0;
    for p in 0..1 << n {
        for q in 0usize..1 << m {
            for src in 0..=n + m {
                if (src < n && src >= 1 && p >> (src - 1) & 1 == 0)
                    || (src >= n + 1 && q >> (src - n - 1) & 1 == 0)
                {
                    continue;
                }
                if dp[p][q][src] == 1e100 {
                ...
                for t in 1..=n + m {
                    ...
                    if t <= n {
                        ...
                        let (tx, ty) = xy[dst];
                        let boost = q.count_ones() as f64;
                        let dist = ((sx - tx) * (sx - tx) + (sy - ty) * (sy - ty)).sqrt()
                            * 2.0f64.powf(-boost);
                        chmin!(dp[p | (1 << dst)][q][dst + 1], dp[p][q][src] + dist);
                    } else {
                        // ブースターのほうをつける
                        let dst = t - n - 1;
                        if q >> dst & 1 == 1 {
                        ...
                        let dist = ((sx - tx) * (sx - tx) + (sy - ty) * (sy - ty)).sqrt()
                            * 2.0f64.powf(-boost);
                        chmin!(dp[p][q | (1 << dst)][dst + n + 1], dp[p][q][src] + dist);
                    }
                }
            }
        }
    ...
    for src in 1..=n + m {
        ...
        for q in 0usize..1 << m {
            let boost = q.count_ones() as f64;
            let dist = (sx * sx + sy * sy).sqrt() * 2.0f64.powf(-boost);
            chmin!(ans, dp[(1 << n) - 1][q][src] + dist);
        }
    }
    io.out(format!("{}\n", ans));
    dbg!(&dp);
