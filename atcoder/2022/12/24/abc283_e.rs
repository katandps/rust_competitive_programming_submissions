...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    ...
    // dp[i][b1][b2] = k => i番目まで見て、i番目が表か裏か、i-1番目が表か裏かになるような回数がk
    let mut dp = vec![vec![vec![INF; 2]; 2]; h + 1];
    // 表 表(裏 裏)
    let mut can = true;
    for x in 0..w {
        if (x > 0 && a[0][x] == a[0][x - 1])
        ...
        for prev_2 in 0..2 {
            ...
            for prev in 0..2 {
                ...
                for next in 0..2 {
                    let mut can = true;
                    for x in 0..w {
                        if (x > 0 && a[y - 1][x] == a[y - 1][x - 1])
                            || (x < w - 1 && a[y - 1][x] == a[y - 1][x + 1])
                            || a[y - 2][x] ^ prev_2 == a[y - 1][x] ^ prev
                            || a[y - 1][x] ^ prev == a[y][x] ^ next
                        {
                            continue;
                        }
                        can = false;
                    ...
                    if can {
                        chmin!(dp[y + 1][next][prev], dp[y][prev][prev_2] + next);
                    }
                }
            }
        }
