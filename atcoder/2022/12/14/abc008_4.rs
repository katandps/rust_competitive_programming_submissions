...
pub fn solve<IO: ReaderTrait + WriterTrait>(io: &mut IO) {
    let (w, h) = io.v2::<usize, usize>();
    let n = io.v::<usize>();
    let xy = io.vec2::<usize, usize>(n);
    let mut memo = HashMap::default();
    io.out(f(1, w + 1, 1, h + 1, (1 << n) - 1, &xy, &mut memo).ln());
}

// set: 残っている装置の集合
// x1..x2, y1..y2: 金塊が配置されている長方形
// xy: 装置が配置されている座標
// memo: (set, x1, x2, y1, y2) -> 個数
fn f(
    x1: usize,
    x2: usize,
    y1: usize,
    ...
    if let Some(r) = memo.get(&(set, x1, x2, y1, y2)) {
        return *r;
    }
    let mut ret = 0;
    for i in 0..xy.len() {
        ...
        let (xi, yi) = xy[i];
        let mut t = x2 - x1 + y2 - y1 - 1;
        let mut v = vec![vec![0, 0]; 2];
        for j in 0..xy.len() {
            if i == j || (1 << j & set) == 0 {
                continue;
            }
            let (xj, yj) = xy[j];
            v[usize::from(xj > xi)][usize::from(yj > yi)] |= 1 << j;
        }
        t += f(x1, xi, y1, yi, v[0][0], xy, memo);
        t += f(x1, xi, yi + 1, y2, v[0][1], xy, memo);
        t += f(xi + 1, x2, y1, yi, v[1][0], xy, memo);
        t += f(xi + 1, x2, yi + 1, y2, v[1][1], xy, memo);
        chmax!(ret, t);
    }
    memo.insert((set, x1, x2, y1, y2), ret);
    ret
}
