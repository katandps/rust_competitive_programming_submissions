...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut s = reader.vec::<u64>(n);
    let mut another = HashMap::new();
    for i in 0..n {
        let p = s[i].prime_factorize();
        let mut cnt = 0;
        let mut cur = 1;
        let mut ano = 1;
        s[i] = 1;
    ...
    for (&n, &c) in &map {
        ...
        ans += if n == 1 {
            ...
            done.insert(*another.get(&n).unwrap());
            max!(c, *map.get(another.get(&n).unwrap()).unwrap_or(&0))
        }
    }
    writer.ln(ans);
}
