...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    q.push_front((0, None));
    while let Some((src, v)) = q.pop_front() {
        if arrived[src] {
            continue;
        }
        if let Some(e) = v {
    ...
    while let Some((src, v)) = q.pop_front() {
        ...
        for (dst, _) in graph.edges(src) {
            ...
            q.push_back((dst, Some((src, dst))));
        }
    }
    for (src, dst) in tree1 {
        writer.ln(format!("{} {}", src + 1, dst + 1));
    }
    for (src, dst) in tree2 {
        writer.ln(format!("{} {}", src + 1, dst + 1));
    }
}
