...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let a = reader.vec::<usize>(n + 1);
    let mvt = ModValTable::<Mi>::new(n + 1);
    let mut cnt = vec![n + 1; n + 1];
    let mut outer = 0;
    for i in 0..n + 1 {
        if cnt[a[i]] != n + 1 {
            outer = cnt[a[i]] + n - i;
    ...
    for i in 1..=n as i64 + 1 {
        writer.ln(mvt.combination(n as i64 + 1, i) - mvt.combination(outer as i64, i - 1));
    }
}
