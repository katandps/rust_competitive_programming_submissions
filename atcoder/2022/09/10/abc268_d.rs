...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for _ in 0..m {
        tmap.insert(reader.chars());
    }

    for p in NextPermutation::from(n) {
        let mut rest = vec![false; n - 1];
        // 可動な_の数
        for _ in 0..16 - s_sum {
            rest.push(true);
        }
        for u in NextPermutation::from(&rest[..]) {
            let mut us = vec![0; n];
            ...
            if !tmap.contains(&x) && x.len() <= 16 && x.len() >= 3 {
                // dbg!(&rest, &us, &s_sum, n - 1);
                return writer.join(&x, "");
            } else {
                // writer.join(&x, "");
            }
        }
