...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut p = reader.vec::<i64>(n);
    for i in 0..n {
        p[i] = (p[i] - i as i64).rem_euclid(n as i64);
    }
    p.sort();
    let s = cumsum(0, &p);
    let mut ans = 1 << 60;
    ...
    // dbg!(&p, &s);
    for k in 0..n as i64 {
        let for_n2 = p.upper_bound(&(n / 2 - k)) as i64;
        let for_n = p.upper_bound(&(n - k)) as i64;
        let for_3n2 = p.upper_bound(&(n + n / 2 - k)) as i64;
        let l1len = for_n2;
        let l2len = for_n - for_n2;
        let l3len = for_3n2 - for_n;
        let l4len = n - for_3n2;

        let l1 = k * l1len + s[for_n2 as usize];
        let l2 = (n - k) * l2len - (s[for_n as usize] - s[for_n2 as usize]);
        let l3 = (k - n) * l3len + (s[for_3n2 as usize] - s[for_n as usize]);
        let l4 = (n * 2 - k) * l4len - (s[n as usize] - s[for_3n2 as usize]);
        // dbg!(for_n2, for_n, for_3n2, l1, l2, l3, l4);
        assert!(l1len + l2len + l3len + l4len == n);

        chmin!(ans, l1 + l2 + l3 + l4);
    }
    writer.ln(ans);
}
