    ...
    for i in 0..n {
        ...
        for j in 0..s[i].len() {
            if s[i][j] == 'X' {
                ...
                let d = (s[i][j] as u8 - b'0') as usize;
                unit_ans += d * cur_x;
                cnt[d] += 1;
            }
        }
        s_stat.push((cur_x, unit_ans, cnt));
    }

    s_stat.sort_by(|sa, sb| {
        let mut sasb = sa.1 + sb.1;
        let mut sbsa = sa.1 + sb.1;
        for i in 1..10 {
            sasb += sb.2[i] * sa.0 * i;
            sbsa += sa.2[i] * sb.0 * i;
        }
