...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut x = 0;
    for &xi in iter.next().unwrap() {
        x *= 10;
        x += (xi as u8 - b'0') as i128;
    }
    let mut y = 0;
    for &yi in iter.next().unwrap() {
        y *= 10;
        y += (yi as u8 - b'0') as i128;
    }
    // dbg!(x, y);
    let a = f(x, y);
    if a.is_empty() {
        return writer.ln("Impossible");
    }
    for (a, b) in a {
...
fn f(mut x: i128, mut y: i128) -> Vec<(i128, i128)> {
    let gcd = Gcd::op(&x, &y);
    x /= gcd;
    y /= gcd;
    let mut ans = Vec::new();
    for n in 2 * x / y - 1..=2 * x / y + 1 {
        let m = (y * n * n + y * n - 2 * x * n) / (2 * y);
        if (y * n * n + y * n - 2 * x * n) % (2 * y) == 0 {
            if 0 < m && m <= n {
                ans.push((n, m))
            }
        }
