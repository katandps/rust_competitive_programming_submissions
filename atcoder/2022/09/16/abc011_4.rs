...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let yd = y / d;
    // dbg!(xd, yd);
    if xd.abs() + yd.abs() > n || (xd.abs() + yd.abs()) % 2 != n % 2 {
        return writer.ln(0);
    }
    let mut ans = 0.0;
    let mut cnts = Vec::new();
    // l方向に何回ジャンプするかを決めると、答えが定まる
    for l in 0..=n {
        let r = xd + l;
        let rest = n - l - r;
        // dbg!(l, r, rest);
        ...
        let u = ((rest + yd) / 2).abs();
        let d = ((rest - yd) / 2).abs();
        if r < 0 || u < 0 || d < 0 || l + r + u + d != n || r - l != xd || u - d != yd {
            continue;
        }
        let mut v = Vec::new();
        for l in 0..l {
    ...
    let mut a = vec![1.0; cnts.len()];
    for i in 0..n {
        for j in 0..cnts.len() {
            a[j] *= i as f64 + 1.0;
            a[j] /= 4.0;
            a[j] /= cnts[j][i as usize] as f64;
        }
    }
    for ai in a {
        ans += ai;
