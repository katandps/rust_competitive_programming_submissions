...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for i in 0..n {
        ...
        max_heap.push(a[n * 3 - 1 - i]);
        rsum += a[n * 3 - 1 - i];
    }
    let mut left = vec![lsum];
    let mut right = vec![rsum];
    for i in 0..n {
        min_heap.push(Reverse(a[i + n]));
        lsum += a[i + n];
        if let Some(Reverse(p)) = min_heap.pop() {
            lsum -= p;
        }
        left.push(lsum);
        max_heap.push(a[n * 2 - 1 - i]);
        rsum += a[n * 2 - 1 - i];
