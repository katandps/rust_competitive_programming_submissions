...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut dp = vec![vec![1 << 30; 1 << n]; n];
    let mut q = VecDeque::new();
    for i in 0..n {
        dp[i][0] = 0;
        q.push_back((i, 0));
    }
    while let Some((src, stat)) = q.pop_front() {
        for (dst, _w) in graph.edges(src) {
            if chmin!(dp[dst][stat ^ (1 << dst)], dp[src][stat] + 1) {
                q.push_back((dst, stat ^ (1 << dst)));
            }
        }
    }
    let mut ans = 0;
