...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let d = reader.vec::<usize>(n);
    let uvw = reader.vec3::<usize, usize, i64>(n - 1);

    let mut graph = Graph::new(n);
    for (u, v, w) in uvw {
        graph.add_edge(u - 1, v - 1, w);
    }
    let (zero, one) = dfs(0, 0, &d, &graph);
    writer.ln(max(zero, one));
}

// (親の辺を使わないときの部分木のスコア, 親の辺を使っているときの部分木のスコア)
fn dfs(src: usize, parent: usize, d: &Vec<usize>, graph: &Graph<i64>) -> (i64, i64) {
    ...
    for (dst, weight) in graph.edges(src) {
        ...
        let (z, o) = dfs(dst, src, d, graph);

        score += z;
        if d[dst] == 0 {
            continue;
        }
        effect.push(o + weight - z);
    }
