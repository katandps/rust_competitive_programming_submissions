...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let t = reader.lowercase();
    let mut alphas = vec![Vec::new(); 26];
    for (i, &si) in s.iter().enumerate() {
        alphas[si].push(i + 1);
    }
    for (i, &si) in s.iter().enumerate() {
        alphas[si].push(i + 1 + s.len());
    }
    let mut cur = 0;
    for ti in t {
        let next = alphas[ti].lower_bound(&(cur % s.len() + 1));
        // dbg!(&alphas[ti], cur, next);

        if alphas[ti].len() == next {
            return writer.ln(-1);
        }
        cur += alphas[ti][next] - cur % s.len();
    }
    writer.ln(cur);
}
