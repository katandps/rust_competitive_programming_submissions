...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let a1 = reader.vec::<usize>(n);
    let a2 = reader.vec::<usize>(n);
    let mut l_sum = vec![0; n];
    let mut r_sum = vec![0; n];
    l_sum[0] = a1[0];
    r_sum[n - 1] = a2[n - 1];
    for i in 1..n {
        l_sum[i] = l_sum[i - 1] + a1[i];
    }
    for i in (0..n - 1).rev() {
        r_sum[i] = r_sum[i + 1] + a2[i];
    }
    let mut ans = 0;
    for i in 0..n {
        chmax!(ans, l_sum[i] + r_sum[i]);
    }
    writer.ln(ans);
}
