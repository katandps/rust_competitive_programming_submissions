...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    if max!(x, y) <= 3 {
        ...
        writer.ln(if max!(x, y) - min!(x, y) <= 1 {
            "Brown"
        } else {
            "Alice"
        })
...
#[test]
fn simulate() {
    let mut memo = HashMap::with_capacity(10000);
    for i in 0..100 {
        for j in 0..=i {
            print!("{}", if dfs(i, j, &mut memo) { "A" } else { "B" });
        }
        println!("{}", i)
    }

    fn dfs(x: usize, y: usize, memo: &mut HashMap<(usize, usize), bool>) -> bool {
        ...
        // xからとる
        for i in 1..x / 2 {
            win |= !dfs(x - i * 2, y + i, memo);
        }
        // yからとる
        for i in 1..y / 2 {
            win |= !dfs(x + i, y - i * 2, memo);
        }
        memo.insert((x, y), win);
        win
    }
