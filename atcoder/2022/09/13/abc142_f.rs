...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for &(a, b) in &ab {
        graph.add_arc(a - 1, b - 1, ());
    }

    let mut ans: Option<Vec<usize>> = None;
    // startから始まる最小サイクルを探す
    for start in 0..n {
        ...
        if let Some(mut dst) = cycle_last {
            let mut a = vec![dst];
            while dst != start {
                for (src, ()) in graph.rev_edges(dst) {
    ...
    if let Some(ans) = ans {
        writer.ln(ans.len());
        writer.join(&ans.iter().map(|v| v + 1).collect::<Vec<_>>(), "\n");
    } else {
        writer.ln(-1)
    }
}
