...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    memo[1][0] = 1;
    let mut patterns = (1usize..1 << n).collect::<Vec<_>>();
    patterns.sort_by_key(|a| a.count_ones());
    for p in patterns {
        for cur in 0..n {
            if p >> cur & 1 == 1 {
                for (dst, ()) in graph.edges(cur) {
                    if p >> dst & 1 == 0 {
                        if memo[p][cur] > 0 {
                            // dbg!(cur, dst, p, p | (1 >> dst));
                        }
                        memo[p | (1 << dst)][dst] += memo[p][cur];
                    }
                }
            }
        }
