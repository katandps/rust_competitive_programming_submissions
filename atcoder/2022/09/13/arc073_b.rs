...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let w: usize = reader.v();
    let wv = reader.vec2::<usize, usize>(n);
    let w1 = wv[0].0;
    if w1 <= 310 {
        let size = 313 * n + 1;
        ...
        for i in 0..n {
            let (w, v) = wv[i];
            // dbg!(&w);
            for j in (0..size - w).rev() {
                chmax!(dp[j + w], dp[j] + v);
            }
        }
        let mut ans = 0;
        for i in 0..=min!(w, size - 1) {
            chmax!(ans, dp[i]);
        }
        writer.ln(ans);
    } else {
        let can = min!(w / w1, n);
        let rest = min!(n * 3, w - can * w1);
        let wv = wv.into_iter().map(|(w, v)| (w - w1, v)).collect::<Vec<_>>();
        let mut dp = vec![vec![0; can + 1]; rest + 1];
        for i in 0..n {
            let (w, v) = wv[i];
            if rest + 1 > w {
                for j in (0..rest + 1 - w).rev() {
                    for k in (0..can).rev() {
                        chmax!(dp[j + w][k + 1], dp[j][k] + v);
                    }
                }
            }
        }
