...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let lr = reader.vec2::<usize, usize>(n);
    const SIZE: usize = 10_000_000;
    let mut left = BinaryIndexedTree::<Addition<i64>>::from(SIZE + 5);
    let mut right = BinaryIndexedTree::<Addition<i64>>::from(SIZE + 5);
    for &(l, r) in &lr {
        left.add(0, 1);
        left.add(l + 1, -1);
        right.add(r + 1, 1);
    }
    let mut left_sum = vec![0; SIZE + 1];
    let mut right_sum = vec![0; SIZE + 3];
    for i in 0..SIZE + 2 {
        right_sum[i + 1] = right_sum[i] + right.sum(i);
    }
    for i in (1..SIZE).rev() {
        left_sum[i - 1] = left_sum[i] + left.sum(i);
    }
    let mut v = 1 << 60;
    let mut j = 0;
    for i in 1..=SIZE {
        if chmin!(v, left_sum[i] + right_sum[i + 1]) {
            j = i;
        }
    }
    let mut p = Vec::new();
