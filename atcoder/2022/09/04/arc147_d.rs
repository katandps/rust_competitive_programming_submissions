...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let n: i64 = reader.v();
    let m: i64 = reader.v();
    writer.ln(mi(n).pow(m) * mi(m).pow(n - 1));

    // 実験の痕跡
    // let mut k = vec![vec![0; 6]; 6];
    // for n in 1..=5 {
    //     for m in 1..=5 {
    //         let mut score = 0;
    //         for start in 0..1 << m {
    //             let a = f(m, n - 1, start, vec![0; m]);
    //             // dbg!(a);
    //             score += a;
    //         }
    //         k[n][m] = score;
    //         println!("n = {} m = {} ans = {}", n, m, score);
    //         writer.ln(score);
    //     }
    // }
    // 1728 4^3 3^3
...
// fn f(m: usize, rest: usize, cur: usize, mut cnt: Vec<usize>) -> usize {
//     for i in 0..m {
//         if (cur >> i) & 1 == 1 {
//             cnt[i] += 1;
//         }
...
//     let mut score = 0;
//     for i in 0..m {
//         score += f(m, rest - 1, cur ^ (1 << i), cnt.clone());
//     }
//     score
// }
