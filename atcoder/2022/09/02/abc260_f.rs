...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (s, t, m) = reader.v3::<usize, usize, usize>();
    let mut uv = reader.vec2::<usize, usize>(m);
    uv.sort();
    uv.push((999999, 999999));

    let mut cur_u = 0;
    let mut stack = Vec::new();
    let mut exist_pair = vec![None; t * t];

    for (u, v) in uv {
        if cur_u != u {
            for i in 0..stack.len() {
                for j in i + 1..stack.len() {
                    let (x, y) = (min(stack[i], stack[j]), max(stack[i], stack[j]));
                    if let Some(u2) = exist_pair.index(x + y * t) {
                        return writer.ln(format!("{} {} {} {}", u2, cur_u, x + s + 1, y + s + 1));
                    }
                    exist_pair[x + y * t] = Some(cur_u)
                }
            }
            cur_u = u;
            stack.clear();
