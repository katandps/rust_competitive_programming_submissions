...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let x = reader.vec::<i64>(n);
    let y = reader.vec::<i64>(n);
    let abz = reader.vec3::<usize, usize, i64>(m);
    let mut ans = 1 << 60;

    let mut resources = vec![Vec::new(); 3];
    for i in 0..n {
        resources[0].push((Reverse(x[i]), i, n));
        resources[1].push((Reverse(y[i]), i, n + 1));
    }
    for &(a, b, z) in &abz {
        resources[2].push((Reverse(z), a - 1, b - 1));
    }
    ...
    for p in 0..1 << 3 {
        ...
        let mut temp = 0;
        while let Some((Reverse(cost), a, b)) = q.pop() {
            if !uf.same(a, b) {
                uf.unite(a, b);
                temp += cost;
            }
        }
        let mut root = (0..n).map(|i| uf.root(i)).collect::<Vec<_>>();
        root.sort();
        root.dedup();
        dbg!(p, temp);
        if root.len() == 1 {
