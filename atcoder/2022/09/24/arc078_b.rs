...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    q.push((Reverse(1), 0));
    // turn 2
    q.push((Reverse(2), n - 1));
    while let Some((Reverse(turn), src)) = q.pop() {
        for (dst, ()) in graph.edges(src) {
            if !black[dst] && !white[dst] {
                if turn % 2 == 1 {
                    black[dst] = true;
                } else {
                    white[dst] = true;
                }
                q.push((Reverse(turn + 2), dst))
            }
        }
    }
    let bl = black.iter().filter(|b| **b).count();
    writer.ln(if bl > n - bl { "Fennec" } else { "Snuke" });
}
