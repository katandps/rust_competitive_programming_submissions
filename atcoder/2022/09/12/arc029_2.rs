...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for _ in 0..n {
        ...
        let theta = binary_search(0.0, PI / 2.0, |t| a * t.sin() + b * t.cos() <= c + EPS);
        // dbg!(
        //     (c, d),
        //     (
        ...
        //     )
        // );
        if a * theta.sin() + b * theta.cos() <= c + EPS
            && a * theta.cos() + b * theta.sin() <= d + EPS
        {
            writer.ln("YES");
            continue;
        }
        let theta = binary_search(0.0, PI / 2.0, |t| b * t.sin() + a * t.cos() <= d + EPS);
        if a * theta.sin() + b * theta.cos() <= c + EPS
            && a * theta.cos() + b * theta.sin() <= d + EPS
        {
            writer.ln("YES");
