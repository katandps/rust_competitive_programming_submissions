...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut a = a.into_iter().map(|ai| ai / 400).collect::<Vec<_>>();
    a.sort();
    let over = a.iter().filter(|k| **k >= 8).count();
    a.dedup();
    let ans = a.iter().filter(|k| **k <= 7).count();

    writer.ln(format!("{} {}", max!(1, ans), ans + over));
}
