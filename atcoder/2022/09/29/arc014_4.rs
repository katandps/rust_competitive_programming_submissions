...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (all, n, m) = reader.v3::<i64, usize, usize>();
    let l = reader.vec::<i64>(n);
    let xy = reader.vec2::<i64, i64>(m);
    let mut d = l.windows(2).map(|v| v[1] - v[0] - 1).collect::<Vec<_>>();
    d.sort();
    let cs = cumsum(0, &d);
    dbg!(&d, &cs);
    for (x, y) in xy {
        ...
        ans += min!(all - l[n - 1], y);
        dbg!(ans);
        let lb = d.lower_bound(&(x + y));
        dbg!(lb);
        ans += cs[lb] + (x + y) * (cs.len() - lb - 1) as i64;
