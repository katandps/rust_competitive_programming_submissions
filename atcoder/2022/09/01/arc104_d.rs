...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mvt = ModValTable::<Mi>::new(k);
    // sigma_{i=1}^n{a_i+a_i}^x
    let mut sq = vec![mi(0); k + 1];

    // sigma_{i=1}^{n}{A^k_i/k!}
    let mut table = vec![mi(0); k + 1];

    for i in 0..n {
        // ModInt::pow()を使うと間に合わない
        let mut tadd = mi(1);
        let mut sadd = mi(1);
        for k in 0..=k {
            table[k] += tadd * mvt.fact_inv[k];
            tadd *= a[i];
            sq[k] += sadd;
            sadd *= a[i] * 2;
        }
    ...
    for x in 1..=k {
        let mut ans = mi(0);
        for i in 0..=x {
            ans += mvt.fact[x] * table[i] * table[x - i];
        }
        writer.ln((ans - sq[x]) / 2);
    }
}
