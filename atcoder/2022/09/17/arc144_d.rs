...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let k: i64 = reader.v();
    let mut ans = mi(0);
    let mut ncm = vec![mi(1)];
    let mut kcm = vec![mi(k + 1)];
    let mut pow2 = vec![mi(1)];
    for i in 1..=n {
        ncm.push(ncm[ncm.len() - 1] * (n + 1 - i) / i);
        kcm.push(kcm[kcm.len() - 1] * (k + 1 - i) / (i + 1));
        pow2.push(pow2[pow2.len() - 1] * 2);
    }

    for m in 0..=n as usize {
        ans += ncm[m] * kcm[m] * pow2[m];
    }
    writer.ln(ans);
}
