...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut uf = UnionFind::new(4004002);
    let mut ans = n as i64;
    for (x, y) in xy {
        if set.contains(&(x - 1, y - 1)) && !uf.same(f(x, y), f(x - 1, y - 1)) {
            ans -= 1;
            uf.unite(f(x, y), f(x - 1, y - 1));
        }
        if set.contains(&(x - 1, y)) && !uf.same(f(x, y), f(x - 1, y)) {
            ans -= 1;
            uf.unite(f(x, y), f(x - 1, y));
        }
        if set.contains(&(x, y - 1)) && !uf.same(f(x, y), f(x, y - 1)) {
            ans -= 1;
            uf.unite(f(x, y), f(x, y - 1));
        }
        if set.contains(&(x, y + 1)) && !uf.same(f(x, y), f(x, y + 1)) {
            ans -= 1;
            uf.unite(f(x, y), f(x, y + 1));
...
fn f(x: i64, y: i64) -> usize {
    ((x + 1000) * 2001 + y + 1000) as usize
}
