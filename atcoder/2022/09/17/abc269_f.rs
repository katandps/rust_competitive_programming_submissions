...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (_n, m, q) = reader.v3::<i64, i64, usize>();
    let abcd = reader.vec4::<i64, i64, i64, i64>(q);
    ...
    //             s[y][x] -= s[y - 1][x - 1];
    //         }
    //         assert_eq!(mi(s[y][x]), sum(m, x as i64, y as i64), "x {} y {}", x, y);
    //     }
    // }
    ...
    for (a, b, c, d) in abcd {
        ...
        let ans = sum(m, d, b) + sum(m, c - 1, a - 1) - sum(m, d, a - 1) - sum(m, c - 1, b);
        writer.ln(ans * per4);
    }
}

fn sum(m: i64, x: i64, y: i64) -> Mi {
    if x < 0 || y < 0 {
        return mi(0);
    }
    let mut ret = mi(0);
