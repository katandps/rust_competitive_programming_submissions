...
impl<R: BufRead, F: FnMut() -> R> Iterator for Reader<F> {
    ...
    fn next(&mut self) -> Option<String> {
        if self.buf.is_empty() {
            ...
            reader.read_line(&mut buf).expect("read fail");
            self.buf
                .append(&mut buf.split_whitespace().map(ToString::to_string).collect());
        }
        self.buf.pop_front()
    }
}
...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, _writer: Writer<W>) {
    let n: usize = reader.v();
    let mut rest = (1..=2 * n + 1).collect::<HashSet<_>>();
    while let Some(&t) = rest.iter().next() {
        rest.remove(&t);
        println!("{}", t);
        let r = reader.v();
        if r == 0 {
