...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let abst = reader.vec4::<usize, usize, usize, usize>(m);
    let xyz = reader.vec3::<usize, usize, usize>(q);

    let mut bus = vec![BTreeMap::new(); n];

    for i in 0..m {
        let (a, _b, s, _t) = abst[i];
        bus[a - 1].insert(s, i);
    }
    // dp[b][i] = バスiを使った後、2^b回移動したときに最後に使ったバス
    let mut dp = vec![vec![m; m + 1]; 20];

    for i in 0..m {
        let (_a, b, _s, t) = abst[i];
        if let Some((_s, &j)) = bus[b - 1].range(t..).next() {
            dp[0][i] = j;
        }
    }
    // dbg!(&dp[0]);
    ...
    for (x, y, z) in xyz {
        if let Some((_s, first)) = bus[y - 1].range(x..).next() {
            let mut bus = *first;
            for bit in (0..20).rev() {
                if dp[bit][bus] == m {
                    continue;
                }
                let (_a, _b, s, _t) = abst[dp[bit][bus]];
                if z > s {
                    bus = dp[bit][bus];
                }
            }
            let (a, b, s, t) = abst[bus];
            // dbg!(&abst[bus], bus, z);
            if z <= s {
                writer.ln(a)
            } else if z > t {
