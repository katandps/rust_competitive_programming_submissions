...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for _ in 0..q {
        let (t, k) = reader.v2::<usize, usize>();
        let k = k - 1;
        let (alpha, index) = if t >= 60 {
            (s[0], k)
        } else {
            (s[k / (1 << t)], k % (1 << t))
        };
        // dbg!(alpha, t, index);
        writer.ln(query(alpha, t, index));
    }
}

fn query(alpha: char, steps: usize, index: usize) -> char {
    if steps > 63 {
        let mut next = steps % 3;
        while 1 << next < index {
            next += 3;
        }
        return query(alpha, next, index);
    }
    if steps == 0 {
        return alpha;
    }
    if index < 1 << (steps - 1) {
        ...
        query(next, steps - 1, index - (1 << (steps - 1)))
    }
}
