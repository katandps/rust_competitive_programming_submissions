...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let c = reader.vec::<usize>(m);
    if m > n.len() {
        return writer.ln(0);
    }

    let mut cmap = vec![None; 10];
    for ci in 0..m {
        cmap[c[ci]] = Some(ci);
    }
    ...
    for i in 0..n.len() {
        ...
        for smaller in 0..2 {
            for bits in 0..1 << m {
                if dp[smaller][bits].get() == 0 && sum[smaller][bits].get() == 0 {
                    continue;
                }
                for next in 0..10 {
                    if smaller == 0 && next > d {
                    ...
                    let next_bits = if let Some(b) = cmap[next] {
                        bits | (1 << b)
                    } else {
                        bits
                    };
