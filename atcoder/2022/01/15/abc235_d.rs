...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let a: usize = reader.v();
    let n: usize = reader.v();
    let mut used = HashMap::new();
    used.insert(1, 0);
    let mut ng = HashSet::new();
    let ans = f(a, n, &mut used, &mut ng);
    if ans >= 1 << 59 {
        writer.ln(-1);
    } else {
        writer.ln(ans);
...
fn f(a: usize, n: usize, used: &mut HashMap<usize, usize>, ng: &mut HashSet<usize>) -> usize {
    // dbg!(n, &ng);
    if let Some(r) = used.get(&n) {
        return *r;
    }
    let mut c = 1 << 60;
    if n % a == 0 {
        chmin!(c, f(a, n / a, used, ng) + 1);
    }
    if n >= 10 {
        let mut n = n;
        let mut ten_pow = 1;
        ...
        ten_pow /= 10;
        if n % ten_pow >= ten_pow / 10 {
            n = n % ten_pow * 10 + n / ten_pow;
            if !ng.contains(&n) {
                ng.insert(n);
                chmin!(c, f(a, n, used, ng) + 1);
            }
        }
    }
    used.insert(n, c);
