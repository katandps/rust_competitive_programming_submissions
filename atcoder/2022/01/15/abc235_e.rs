...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, m, q): (usize, usize, usize) = reader.v3();
    let abc = reader.vec3::<usize, usize, i64>(m);
    let uvw = reader.vec3::<usize, usize, i64>(q);

    let mut ans = vec![false; q];

    // events = (Reverse(weight), (is_add, a, b, index));
    let mut events = BinaryHeap::new();
    for (a, b, c) in abc {
        events.push((Reverse(c), (true, a - 1, b - 1, 0)))
    }
    for i in 0..q {
        let (u, v, w) = uvw[i];
        events.push((Reverse(w), (false, u - 1, v - 1, i)))
    }

    let mut uf = UnionFind::new(n);
    while let Some((Reverse(_weight), (is_add, src, dst, index))) = events.pop() {
        if is_add {
            uf.unite(src, dst);
        } else {
            if !uf.same(src, dst) {
