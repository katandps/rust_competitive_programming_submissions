...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    // dp[i][p] = patterns; 0: a only, 1: b only, 2: both included
    let mut dp = vec![vec![mi(0); 3]; n];
    writer.ln(f(0, 0, &graph, &c, &mut dp));
    // dbg!(dp);
}
fn f(src: usize, parent: usize, graph: &Graph<()>, nodes: &Vec<char>, dp: &mut Vec<Vec<Mi>>) -> Mi {
    let mut two_pos = mi(1);
    let mut two_neg = mi(1);
    if nodes[src] == 'a' {
        let mut zero = mi(1);
        for (dst, _) in graph.edges(src) {
            ...
            f(dst, src, graph, nodes, dp);
            zero *= dp[dst][0] + dp[dst][2];
            two_pos *= dp[dst][0] + dp[dst][1] + dp[dst][2] * 2;
            two_neg *= dp[dst][0] + dp[dst][2];
        }
        dp[src][0] = zero;
    } else {
        let mut one = mi(1);
        ...
        for (dst, _) in graph.edges(src) {
            ...
            f(dst, src, graph, nodes, dp);
            one *= dp[dst][1] + dp[dst][2];
            two_pos *= dp[dst][0] + dp[dst][1] + dp[dst][2] * 2;
            two_neg *= dp[dst][1] + dp[dst][2];
        }
        dp[src][1] = one;
    }
    dp[src][2] = two_pos - two_neg;
    dp[src][2]
}
