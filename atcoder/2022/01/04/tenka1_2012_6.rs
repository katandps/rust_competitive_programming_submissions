...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut back_trim = 0;
    while s[front_trim] == '_' {
        front_trim += 1;
        if front_trim == s.len() {
            return writer.join(&original, "");
        }
    }
    while s[s.len() - 1 - back_trim] == '_' {
        back_trim += 1;
    }
    let mut count = 0;
    let mut to_snake = Vec::new();
    let mut camel = Vec::new();
    for word in s[front_trim..s.len() - back_trim].split(|c| *c == '_') {
        count += 1;
        if count == 1 {
            to_snake = word.to_vec();
        }
        if word.is_empty() || !word[0].is_lowercase() {
            return writer.join(&original, "");
        }
        if camel.is_empty() {
            camel.push(word.to_vec());
        } else {
    ...
    let mut ans = Vec::new();
    for _ in 0..front_trim {
        ans.push('_');
    }
    if count == 1 {
        for mut char in to_snake {
