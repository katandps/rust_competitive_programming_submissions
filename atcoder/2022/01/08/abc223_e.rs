...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (x, y, a, b, c): (usize, usize, usize, usize, usize) = reader.v5();
...
fn check(x: usize, y: usize, a: usize, b: usize, c: usize) -> bool {
    if (a + x - 1) / x >= y {
        return false;
    }
    let rest_y = y - (a + x - 1) / x;
    (b + rest_y - 1) / rest_y + (c + rest_y - 1) / rest_y <= x
}
