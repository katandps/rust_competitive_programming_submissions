...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mvt = ModValTable::new(6000);

    let mut js = vec![vec![mi(0); n + 1]; n + 1];
    for j in 0..=n {
        for s in 0..=n - j {
            js[j][s] = mvt.factorial(j as i64 + s as i64) * mvt.fact_inv[j] * mvt.fact_inv[s]
        }
    }

    // dp[i][j] = i種類目まで見て、長さがjになっている列の種類数
    let mut dp = vec![vec![mi(0); n + 1]; 27];
    dp[0][0] += 1;
    for i in 0..26 {
        // s文字入れる
        for s in 0..=counts[i] {
