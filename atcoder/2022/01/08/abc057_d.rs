...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, a, b): (usize, usize, usize) = reader.v3();
    let v = reader.vec::<usize>(n);
    //dp[i][j] = i番目まで見てj個選んでいるものの合計の最大値と、その個数
    let mut dp_max = vec![vec![0; n + 1]; n + 1];
    let mut dp_count = vec![vec![0usize; n + 1]; n + 1];
    dp_count[0][0] = 1;
    for i in 0..n {
        for j in 0..=n {
            dp_count[i + 1][j] = dp_count[i][j];

            chmax!(dp_max[i + 1][j], dp_max[i][j]);

            if j > 0 {
                if chmax!(dp_max[i + 1][j], dp_max[i][j - 1] + v[i]) {
                    dp_count[i + 1][j] = 0;
                }
                if dp_max[i + 1][j] == dp_max[i][j - 1] + v[i] {
                    dp_count[i + 1][j] += dp_count[i][j - 1];
                }
            }
        }
    }
    ...
    writer.ln(val.0 as f64 / val.1 as f64);
    writer.ln(ans);
}
