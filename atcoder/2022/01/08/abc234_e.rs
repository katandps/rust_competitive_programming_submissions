...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut ten_power = vec![1i64];
    for i in 0..18 {
        ten_power.push(ten_power[i] * 10);
    }

    // 先頭文字が同じ 差は+-9まで
    'c: for diff in -9..=9 {
        let mut v = vec![xdigit[0]];
        for i in 1..xdigit.len() {
            if v[i - 1] + diff < 0 || v[i - 1] + diff >= 10 {
                continue 'c;
            }
            v.push(v[i - 1] + diff);
        }
        let mut y = 0;
        for i in 0..xdigit.len() {
            y += v[xdigit.len() - 1 - i] * ten_power[i];
        }
        if y >= x {
            anss.push(y);
        }
