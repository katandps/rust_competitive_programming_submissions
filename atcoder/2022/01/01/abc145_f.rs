...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let h_ = reader.vec::<i64>(n);
    let mut h = vec![0];
    for hi in h_ {
        h.push(hi);
    }
    let mut dp = vec![vec![vec![1 << 60; n + 1]; n + 1]; n + 1];

    // dp[i][j][p] i番目まで見て最後に採用したマスがjであり、合計p個スキップしているときの操作回数の最小値
    dp[0][0][0] = 0;
    ...
    for i in 0..n {
        for j in 0..n {
            for p in 0..n {
                // スキップしない
                chmin!(dp[i + 1][i + 1][p], dp[i][j][p] + max(0, h[i + 1] - h[j]));
                // スキップする
                chmin!(dp[i + 1][j][p + 1], dp[i][j][p]);
            }
        }
    }
    let mut ans = 1 << 60;
