...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (h, w, k): (usize, usize, usize) = reader.v3();
    let a = reader.vec::<usize>(h);
    let b = reader.vec::<usize>(w);
    let mut ans = (h - 1) * (w - 1) * (k - 1);
    let rowsum = (w - 1) * (k - 1) % k;
    let mut a_sum = 0;
    for i in 0..h - 1 {
        let ai = a[i];
        a_sum += (ai + k - rowsum) % k
    }
    let colsum = (h - 1) * (k - 1) % k;
    let mut b_sum = 0;
    for i in 0..w - 1 {
        let bi = b[i];
        b_sum += (bi + k - colsum) % k;
    ...
    ans += a_sum + b_sum;
    let mut rest = (h - 1) * (w - 1) * (k - 1);
    let a_add = (h - 1) * (k - 1) - a_sum;
    let b_add = (w - 1) * (k - 1) - b_sum;
    let add = min(rest, min(a_add, b_add));
    ans += add;
    a_sum += add;
    b_sum += add;

    let last = (b[w - 1] + k - a_sum % k) % k;
    // dbg!(last);
    ans += last;
    if (b_sum + last) % k != a[h - 1] {
        writer.ln(-1);
    } else {
        writer.ln(ans);
    }
