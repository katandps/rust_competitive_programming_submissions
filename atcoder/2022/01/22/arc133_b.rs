...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let q = reader.vec::<usize>(n);

    let mut spots = vec![BTreeSet::new(); n + 1];
    for i in 0..n {
        let t = q[i];
        let mut l = 1;
        while l * l <= t {
            if t % l == 0 {
                spots[l].insert(Reverse(i));
                spots[t / l].insert(Reverse(i));
            }
            l += 1;
        }
    }

    let mut segtree = SegmentTree::<Maximization<i64>>::from(&vec![0; n + 1]);

    for i in 1..=n {
        let pi = p[i - 1];
        for &Reverse(qi) in &spots[pi] {
            // dbg!(qi, segtree.prod(0..=qi));
            if segtree.prod(qi + 1..=qi + 1) < segtree.prod(0..=qi) + 1 {
                segtree.update_at(qi + 1, segtree.prod(0..=qi) + 1);
            }
        }
    }

    writer.ln(segtree.prod(1..=n));
}
...
impl<M: Monoid> SegmentTree<M> {
    pub fn new(n: usize) -> Self {
        ...
        let _log = n.trailing_zeros() as usize;
        let node = vec![M::unit(); 2 * n];
        let mut segtree = Self { n, _log, node };
        for i in (0..segtree.n - 1).rev() {
            segtree.node[i] = M::op(&segtree.node[2 * i], &segtree.node[2 * i + 1]);
        }
        segtree
