...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let a = reader.vec::<usize>(3);
    let b = reader.vec::<usize>(3);
...
fn f(src: usize, cost: &[usize], gain: &[usize]) -> usize {
    let mut ret = src;
    match cost.len() {
        0 => (),
        1 => {
            chmax!(ret, src / cost[0] * gain[0] + src % cost[0]);
        }
        2 => {
            for first in 0..=src / cost[0] {
                let mut rest = src - first * cost[0];
                let second = rest / cost[1];
                rest %= cost[1];
                chmax!(ret, first * gain[0] + second * gain[1] + rest);
            }
        }
        _ => {
            for first in 0..=src / cost[0] {
                for second in 0..=(src - first * cost[0]) / cost[1] {
                    let mut rest = src - first * cost[0] - second * cost[1];
                    let third = rest / cost[2];
                    rest %= cost[2];
                    chmax!(
                        ret,
                        first * gain[0] + second * gain[1] + third * gain[2] + rest
                    );
                }
            }
        }
