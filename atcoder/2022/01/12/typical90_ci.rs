...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, p, k) = reader.v3::<usize, i64, usize>();
    let a = reader.matrix::<i64>(n, n);

    const INF: i64 = 1 << 60;
    // ルートがk個以上になるようなxの最大値
    let max = binary_search(0, INF, |mid| {
        ...
        let graph = GraphMatrix::from(matrix);
    ...
    // ルートがk個より大きくなるようなxの最大値
    let over = binary_search(0, INF, |mid| {
        let mut matrix = a.clone();
        for i in 0..n {
            for j in 0..n {
                if matrix[i][j] == -1 {
