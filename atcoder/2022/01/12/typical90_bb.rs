...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    // 人が書いた論文
    let mut scholars = vec![Vec::new(); n];
    for mi in 0..m {
        let k = reader.v();
        let mut r = reader.vec::<usize>(k);
        for i in 0..k {
            r[i] -= 1;
            scholars[r[i]].push(mi);
        }
        papers.push(r);
    }
    ...
    ans[0] = 0;
    // 論文をもう見たか
    let mut paper_is_used = vec![false; m];

    let mut q = VecDeque::new();
    q.push_front(0);
    while let Some(scho) = q.pop_front() {
        for &paper in &scholars[scho] {
            if paper_is_used[paper] {
                continue;
            }
            paper_is_used[paper] = true;
            for &next in &papers[paper] {
                if ans[next] == -1 {
                    ans[next] = ans[scho] + 1;
                    q.push_back(next);
                }
