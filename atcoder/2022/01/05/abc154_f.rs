...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (r1, c1, r2, c2): (i64, i64, i64, i64) = reader.v4();

    let mvt: ModValTable<Mi> = ModValTable::new(2000002);
    let f = |r: i64, c: i64| {
        let mut ret = mi(0);
        for i in 1..=(r + 1) {
            ret += mvt.combination(c + i, i);
        }
        ret
    };

    writer.ln(f(r2, c2) - f(r1 - 1, c2) - f(r2, c1 - 1) + f(r1 - 1, c1 - 1));
}
