...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, q): (usize, usize) = reader.v2();
    let a = reader.vec::<usize>(n);
    let xy = reader.vec2::<usize, usize>(q);
    let mut graph = vec![0i128; n];
    for i in 0..q {
        let (x, y) = xy[i];
        graph[x - 1] |= 1 << (y - 1);
    }
    let mut ans = vec![HashSet::new(); 8889];
    let a = f(0, 0, 0, 0, &a, &graph, &mut ans);
    let ans = ans[a].iter().cloned().collect::<Vec<i128>>();

    writer.ln(ans[0].count_ones());
    let mut k = Vec::new();
    for i in 0..90 {
        if ans[0] >> i & 1 == 1 {
            k.push(i + 1);
...
fn f(
    ...
    if 1i128 << next & ng == 0 {
        max(
            ...
            f(cur, next + 1, sum, ng, a, graph, ans),
        )
    } else {
        f(cur, next + 1, sum, ng, a, graph, ans)
    }
