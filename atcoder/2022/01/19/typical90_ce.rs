...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, m): (usize, usize) = reader.v2();
    let ab = reader.vec2::<usize, usize>(m);
    let mut graph = Graph::new(n);
    for &(a, b) in &ab {
        graph.add_edge(a - 1, b - 1, ());
    }
    let threshold = sqrt(m as i64) as usize;
    let mut large_graph = Graph::new(n);
    let mut is_large = vec![false; n];
    for i in 0..n {
        if graph.edges(i).len() > threshold {
            is_large[i] = true;
        }
    }
    for (a, b) in ab {
        if graph.edges(a - 1).len() > threshold && graph.edges(b - 1).len() > threshold {
            large_graph.add_edge(a - 1, b - 1, ());
        }
    }
    let q: usize = reader.v();
    let mut color = vec![1; n];
    let mut last = vec![0; n];
    let mut last_color = vec![1; n];
    for i in 0..q {
        ...
        if is_large[x] {
            // dbg!(&color);
            writer.ln(color[x]);
            for (dst, ()) in large_graph.edges(x) {
                color[dst] = y;
            }
            last[x] = i + 1;
            color[x] = y;
