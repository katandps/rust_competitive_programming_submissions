...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut map = vec![vec![0; 26]; 3];
    let n = s1.len();
    for si in s1 {
        map[0][(si as u8 - b'A') as usize] += 1;
    }
    for si in s2 {
        map[1][(si as u8 - b'A') as usize] += 1;
    }
    for si in s3 {
        map[2][(si as u8 - b'A') as usize] += 1;
    }
    for i in 0..26 {
        let must_0 = max(0, map[2][i] - map[1][i]);
        let must_1 = max(0, map[2][i] - map[0][i]);
        if map[0][i] < must_0 || map[1][i] < must_1 {
            return writer.ln("NO");
        }
        map[0][i] -= must_0;
        map[1][i] -= must_1;
    ...
    if sum[0] < n as i32 / 2 || sum[1] < n as i32 / 2 {
        writer.ln("NO")
    } else {
        writer.ln("YES")
    }
