...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (q, l): (usize, usize) = reader.v2();
    let mut vq = VecDeque::new();
    let mut size = 0;
    for _ in 0..q {
        let q: String = reader.v();
        match q.as_str() {
            "Push" => {
                let (n, m): (usize, i64) = reader.v2();
                size += n;
                if size > l {
                    return writer.ln("FULL");
                }
                vq.push_front((n, m));
            }
            "Pop" => {
                let mut rest: usize = reader.v();
                while rest > 0 {
                    if let Some((mut n, m)) = vq.pop_front() {
                        if n > rest {
                            n -= rest;
                            size -= rest;
                            rest = 0;
                        ...
                        return writer.ln("EMPTY");
                    }
                }
            }
            "Top" => {
                if let Some((n, m)) = vq.pop_front() {
                    writer.ln(m);
                    vq.push_front((n, m));
                } else {
                    return writer.ln("EMPTY");
