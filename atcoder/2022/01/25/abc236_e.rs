...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let median = binary_search(0, 1_000_000_001, |mid| {
        let (s, t) = a.iter().fold((0, 0), |(s, t), &ai| {
            (max(s, t) + if ai >= mid { 1 } else { -1 }, s)
        });
        max(s, t) > 0
    });
    let average = binary_search(0, 1_000_000_000_001, |mid| {
        let (s, t) = a
            .iter()
            .fold((0, 0), |(s, t), &ai| (max(s, t) + ai * 1000 - mid, s));
        max(s, t) >= 0
    });
    writer.ln(average as f64 / 1000.0);
    writer.ln(median);
}
