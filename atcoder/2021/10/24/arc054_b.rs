...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let (mut lower, mut upper) = (0.0, p);

    fn k(x: f64, p: f64) -> f64 {
        p / 2.0f64.powf(x / 1.5)
    }

    while (upper - lower).abs() > 1e-9 {
        // mid年待つ
        let mid1 = (lower * 2.0 + upper) / 3.0;
        let mid2 = (lower + 2.0 * upper) / 3.0;
        if k(mid1, p) + mid1 > k(mid2, p) + mid2 {
            lower = mid1;
        } else {
            upper = mid2;
        }
    }
    let ans = lower + k(lower, p);
    writer.println(&ans);
}
