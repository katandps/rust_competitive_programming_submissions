...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut dp = vec![mi(0); MAX];
    dp[0] = mi(1);
    for i in 0..n - 1 {
        for dst in (count[i]..MAX).rev() {
            let l = dp[dst - count[i]];
            dp[dst as usize] += l;
        }
    }
    let ans = k + sum;
