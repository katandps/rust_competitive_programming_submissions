...
impl RPS {
    ...
    pub fn is_win(self, rhs: Self) -> bool {
        use RPS::*;
        match (self, rhs) {
            (Rock, Rock) => false,
            (Rock, Paper) => false,
...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (n, m) = reader.u2();
    let a = reader.char_map(2 * n);
    let a: Vec<Vec<RPS>> = a
        .into_iter()
        .map(|ai| ai.into_iter().map(|aii| aii.into()).collect())
        .collect();

    let mut rank: Vec<_> = (0..n * 2).collect();
    let mut wins = vec![0; n * 2];
    for i in 0..m {
        for k in 0..n {
            let left = rank[2 * k];
            let right = rank[2 * k + 1];
            if a[left][i].is_win(a[right][i]) {
                wins[left] += 1;
            }
            if a[right][i].is_win(a[left][i]) {
                wins[right] += 1;
            }
        }
        rank.sort_by_key(|&i| (Reverse(wins[i]), i));
    }
    for i in 0..2 * n {
        rank[i] += 1;
    }
    writer.print_join(&rank, None);
}
