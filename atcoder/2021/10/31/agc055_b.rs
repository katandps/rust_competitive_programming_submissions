...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    writer.println(if calc(&s) == calc(&t) { "Yes" } else { "No" });
}

fn calc(str: &[i64]) -> Vec<i64> {
    let mut rest = Vec::new();
    for i in 0..str.len() {
        rest.push(i);
        while rest.len() >= 3
            && str[rest[rest.len() - 1]] == str[rest[rest.len() - 2]]
            && str[rest[rest.len() - 1]] == str[rest[rest.len() - 3]]
        {
