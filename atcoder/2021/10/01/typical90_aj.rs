...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut output: Writer<W>) {
    ...
    let xy = xy
        ...
        .map(|(x, y)| (x - y, x + y))
        .collect::<Vec<_>>();
    let x_max = xy.iter().map(|(x, _y)| *x).max().unwrap();
    let y_max = xy.iter().map(|(_x, y)| *y).max().unwrap();
    let x_min = xy.iter().map(|(x, _y)| *x).min().unwrap();
    let y_min = xy.iter().map(|(_x, y)| *y).min().unwrap();
    for _ in 0..q {
        let q = reader.u() - 1;
        let (x, y) = xy[q];
        let v = vec![
        ...
        output.println(v.iter().max().unwrap());
    }
}
