...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut dp = vec![vec![None; b + 1]; a + 1];
    f(a, b, a, b, &av, &bv, &mut dp);
    writer.println(&(dp[a][b].unwrap()));
}
...
fn f(
    ...
    if let Some(r) = dp[rest_a][rest_b] {
        return r;
    }
    let snuke_turn = (a - rest_a + b - rest_b) & 1 == 0;
    let r = if rest_a == 0 {
        if rest_b == 0 {
            0
        } else {
            f(a, b, rest_a, rest_b - 1, av, bv, dp) + if snuke_turn { bv[b - rest_b] } else { 0 }
        }
    } else {
        if rest_b == 0 {
            f(a, b, rest_a - 1, rest_b, av, bv, dp) + if snuke_turn { av[a - rest_a] } else { 0 }
        } else {
            if snuke_turn {
                max(
                    f(a, b, rest_a - 1, rest_b, av, bv, dp) + av[a - rest_a],
                    f(a, b, rest_a, rest_b - 1, av, bv, dp) + bv[b - rest_b],
                )
            } else {
                min(
                    f(a, b, rest_a - 1, rest_b, av, bv, dp),
                    f(a, b, rest_a, rest_b - 1, av, bv, dp),
                )
            }
        }
    };
    dp[rest_a][rest_b] = Some(r);
    r
}
