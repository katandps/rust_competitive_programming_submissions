...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut output: Writer<W>) {
    let t = reader.u();
    for _ in 0..t {
        let (mut two, mut three, mut four) = reader.u3();
        let mut ans = 0;
        let a = min(three / 2, four); // 3, 3, 4
        ans += a;
        three -= a * 2;
        four -= a;
        let a = min(four / 2, two); // 4, 4, 2
        ans += a;
        two -= a;
        four -= a * 2;
        let a = min(three / 2, two / 2); // 3, 3, 2, 2
        ans += a;
        // three -= a * 2;
        two -= a * 2;
        let a = min(four, two / 3); // 4, 2, 2, 2
        two -= a * 3;
        ans += a;
        ans += two / 5;
        output.println(&ans);
