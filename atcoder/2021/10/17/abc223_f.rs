...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (n, q) = reader.u2();
    let mut min_tree = LazySegmentTree::<Mono>::from(&vec![0; n + 1]);
    let s = reader.s();
    let mut c = s
        .into_iter()
        .map(|si| if si == '(' { 1 } else { -1 })
        .collect::<Vec<i64>>();
    min_tree.update_at(0, 0);
    for i in 0..n {
        min_tree.update_range(i + 1..=n, c[i]);
    }
    for _ in 0..q {
        match reader.u() {
            1 => {
                ...
                if l == r {
                    ...
                    min_tree.update_range(lr.0.., -2);
                    min_tree.update_range(lr.1.., 2);
                } else {
                    min_tree.update_range(lr.0.., 2);
                    min_tree.update_range(lr.1.., -2);
                }
            ...
            2 => {
                let (l, r) = reader.u2();
                let ll = min_tree.get(l - 1);
                // for i in 0..=n {
                //     println!("{}; {}", i, min_tree.get(i));
                // }
                //
                // dbg!(l, r, ll, min_tree.get(r), min_tree.prod(l..=r));
                if ll == min_tree.get(r) && min_tree.prod(l..=r) == ll {
                    writer.println(&"Yes");
                } else {
                    writer.println(&"No");
                }
