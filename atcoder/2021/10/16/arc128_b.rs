...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let t = reader.u();
    for _ in 0..t {
        let mut c = reader.iv(3);
        c.sort();
        let mut ans = Vec::new();
        if (c[2] - c[1]) % 3 == 0 {
            ans.push((c[2] - c[1]) / 3 + c[1] + (c[2] - c[1]) * 2 / 3);
        }
        if (c[1] - c[0]) % 3 == 0 {
            ans.push((c[1] - c[0]) / 3 + c[0] + (c[1] - c[0]) * 2 / 3);
        }
        if (c[2] - c[0]) % 3 == 0 {
            ans.push((c[2] - c[0]) / 3 + c[0] + (c[2] - c[0]) * 2 / 3);
        }
        if ans.is_empty() {
            writer.println(&-1);
        } else {
            writer.println(&ans.iter().min().unwrap());
        }
    }
}
