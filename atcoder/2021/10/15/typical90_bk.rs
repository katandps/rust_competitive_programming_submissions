...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (h, w) = reader.u2();
    let p = reader.matrix(h, w);

    let mut ans = 1;
    let mut dp = vec![vec![0; 1 << h]; h * w + 1];
    for x in 0..w {
        'case: for pat in 1..1 << h {
            let v = (0..h)
                .filter(|&i| ((pat >> i) & 1) == 1)
                .collect::<Vec<_>>();
            for i in 1..v.len() {
                if p[v[i]][x] != p[v[i - 1]][x] {
                    continue 'case;
                }
            }
            dp[p[v[0]][x] as usize][pat] += 1;
            chmax!(ans, dp[p[v[0]][x] as usize][pat] * v.len());
        }
    }
    writer.println(&ans);
}
