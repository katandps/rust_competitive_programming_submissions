...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    dist.insert(vec![1, 2, 3, 4, 5, 6, 7, 8, 9], 0);

    let mut q = VecDeque::new();
    q.push_back((vec![1, 2, 3, 4, 5, 6, 7, 8, 9], 8));
    while let Some((v, empty)) = q.pop_front() {
        let &from_d = dist.get(&v).unwrap();
        for &to in &graph[empty] {
            let mut vv = v.clone();
            vv.swap(to, empty);
            if let Some(&d) = dist.get(&vv) {
                if d >= from_d {
                    continue;
                }
            } else {
                dist.insert(vv.clone(), from_d + 1);
                q.push_back((vv, to));
            }
        }
    }
