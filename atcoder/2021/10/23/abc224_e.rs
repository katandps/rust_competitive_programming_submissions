...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let rca = reader.uv3(n);

    let mut xv = vec![BTreeMap::new(); w + 1];
    let mut yv = vec![BTreeMap::new(); h + 1];

    for i in 0..n {
        let (r, c, a) = rca[i];
        xv[c].entry(a).or_insert(Vec::new()).push(r);
        yv[r].entry(a).or_insert(Vec::new()).push(c);
    }

    let mut dp_x = HashMap::new();
    let mut dp_y = HashMap::new();

    for i in 0..n {
        let (r, c, a) = rca[i];
        writer.println(&dfs(c, r, a, &xv, &yv, &mut dp_x, &mut dp_y));
    }
}
...
fn dfs(
    ...
    y: usize,
    a: usize,
    xv: &Vec<BTreeMap<usize, Vec<usize>>>,
    yv: &Vec<BTreeMap<usize, Vec<usize>>>,
    dp_x: &mut HashMap<(usize, usize), usize>,
    dp_y: &mut HashMap<(usize, usize), usize>,
) -> usize {
    let dx = if let Some(&d) = dp_x.get(&(x, a)) {
        d
    } else {
        let mut ret = 0;
        xv[x]
            ...
            .iter()
            .for_each(|(&to_a, to_y)| {
                to_y.iter().for_each(|y| {
                    chmax!(ret, dfs(x, *y, to_a, xv, yv, dp_x, dp_y) + 1);
                })
            });
        dp_x.insert((x, a), ret);
        ret
    };
    let dy = if let Some(&d) = dp_y.get(&(y, a)) {
        d
    } else {
        let mut ret = 0;
        yv[y]
