...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (n, a) = (reader.val::<usize>(), reader.val::<usize>());
    let k = reader.digits();
    let b = reader.vec::<usize>(n);
    let mut arrive = vec![Vec::new(); 2];
    ...
    if flag {
        l += arrive[1].len() - len % arrive[1].len();
        l %= arrive[1].len();
    }
    if !flag {
        writer.println(arrive[0][l]);
    } else {
        writer.println(arrive[1][l]);
    }
}
