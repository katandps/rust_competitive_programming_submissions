...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut right = n;
    while right > 0 {
        let mut max = (a[right - 1], 1); //(sum, count)
        let mut sum = a[right - 1];
        for count in 2..=right {
            sum += a[right - count];
            if max.0 * count < sum * max.1 {
                max = (sum, count);
            }
        }
        if max.1 * m > rest {
            ans += max.0 as f64 * rest as f64 / max.1 as f64;
            break;
        } else {
            ans += max.0 as f64 * m as f64;
            rest -= max.1 * m;
            right -= max.1;
        }
    }
