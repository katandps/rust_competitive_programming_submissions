...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let goal = reader.s();
    let n = reader.u();
    let words: Vec<_> = (0..n).map(|_| reader.s()).collect();
    ...
    for i in 0..n {
        let word = &words[i];
        let mut changes_start = 0;
        let mut changes_goal = 0;
        for i in 0..word.len() {
            if word[i] != start[i] {
                changes_start += 1;
            }
    ...
    if changes < 2 {
        graph.add_edge(n, n + 1, changes);
    }

    let dijkstra = Dijkstra::calc(&graph, n);
    let path = dijkstra.path(n + 1);
    if start != goal && path.len() < 2 {
        writer.println(&-1);
        return;
    }
    writer.println(&(path.len() - 2));
    writer.println(&(start.into_iter().collect::<String>()));
    path[1..path.len() - 1]
        .iter()
        .for_each(|i| writer.println(&words[*i].iter().collect::<String>()));
    writer.println(&(goal.into_iter().collect::<String>()));
}
