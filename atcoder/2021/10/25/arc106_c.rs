...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut ans = Vec::new();
    if m > 0 {
        ans.push((10, 100000000));
        for i in 1..=m.abs() {
            ans.push((i * 20, i * 20 + 10));
        }
        ans.push((99999000, 100000010));
    }
    for i in 0..n - ans.len() as i64 {
        ans.push((10000020 + i * 20, 100000030 + i * 20));
    }

    for (a, b) in ans {
        writer.println(&format!("{} {}", a, b));
    }
}
