...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut output: Writer<W>) {
    ////
    let n = reader.u();
    let mut lr = reader.iv2(n);
    lr.sort_by_key(|&(l, r)| (l, Reverse(r)));
    lr = lr.into_iter().map(|(l, r)| (l, r + 1)).collect();

    const INF: i64 = 1 << 60;
    let mut s = vec![(-INF, INF); n + 1];
    for i in 0..n {
        let (start, end) = s[i];
        let (l, r) = lr[i];
        s[i + 1] = (start.max(l), end.min(r));
    }
    let mut s_rev = vec![(-INF, INF); n + 1];
    for i in (0..n).rev() {
        let (start, end) = s_rev[i + 1];
        let (l, r) = lr[i];
        s_rev[i] = (start.max(l), end.min(r));
    }

    let mut ans = 0;
    for left in 1..n {
        let (ll, lr) = s[left];
        let (rl, rr) = s_rev[left];
        ans = ans.max((lr - ll).max(0) + (rr - rl).max(0));
    }
    ...
    for i in 1..(n - 1) {
        ...
        let (sl, sr) = (ll.max(rl), lr.min(rr));
        ans = ans.max((r - l).max(0) + (sr - sl).max(0));
    }
    output.println(&ans);
}
