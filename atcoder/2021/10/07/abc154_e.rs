...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut output: Writer<W>) {
    ...
    let d = n.len() as i64;
    if k == 1 {
        output.println(&f1(&n, d));
    } else if k == 2 {
        if d == 1 {
            output.println(&0)
        } else {
            output.println(&f2(&n, d));
        }
    } else {
        if d < 3 {
            output.println(&0)
        } else {
            output.println(&f3(&n, d));
        }
    }
}

fn f1(mut n: &[i64], mut d: i64) -> i64 {
    while d != 0 && n[0] == 0 {
        n = &n[1..];
        d -= 1;
    }
