...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    for _ in 0..q {
        match reader.val::<usize>() {
            1 => {
                let (x, y) = (reader.val::<usize>(), reader.val::<usize>());
                nodes[x].back = Some(y);
                nodes[y].front = Some(x);
            }
            2 => {
                let (x, y) = (reader.val::<usize>(), reader.val::<usize>());
                nodes[x].back = None;
            ...
            3 => {
                let x = reader.val::<usize>();
                let mut ans = VecDeque::new();
                ans.push_back(x);
                let mut front = x;
                while let Some(f) = nodes[front].front {
                    ans.push_front(f);
                    front = f;
                }
                let mut back = x;
                while let Some(b) = nodes[back].back {
                    ans.push_back(b);
                    back = b;
                }
                ans.push_front(ans.len());
                let ans: Vec<_> = ans.into_iter().collect();
