...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (k, q) = (reader.val::<usize>(), reader.val::<usize>());
    let d: Vec<i64> = reader.vec(k);
    for _ in 0..q {
        let (n, x, m) = (
            reader.val::<i64>(),
            reader.val::<i64>(),
        ...
        let mut c = vec![0];
        for i in 0..k {
            c.push(c[i] + if d[i] % m != 0 { 1 } else { 0 });
            s.push(s[i] + d[i] % m);
        }
        let n = n - 1;
        let last = x % m + n / k as i64 * s[k] + s[n as usize % k];
        let c2 = n / k as i64 * c[k] + c[n as usize % k];

        writer.println(c2 - last / m);
    }
}
