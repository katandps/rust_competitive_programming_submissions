...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    {
        let mut set = (1..=n as i64).collect::<HashSet<_>>();
        for i in 0..n {
            set.remove(&a[i]);
        }
        for k in set {
    ...
    dp.insert(Vec::new(), mi(1));
    for i in 0..n as i64 {
        // dbg!(&dp);

        let mut new_keep = Vec::new();
        while let Some(Reverse(pi)) = rest.pop() {
            if pi <= i + d + 1 {
                new_keep.push(pi);
            } else {
                rest.push(Reverse(pi));
                break;
            }
        }
        // dbg!(&new_keep);
        ...
        for (keep, val) in &dp {
            ...
            // dbg!(i, &keep, &val, a[i as usize]);
            if a[i as usize] != -1 {
                next.insert(keep, val);
            } else if keep.len() == 0 {
                ()
            } else if keep[0] < i + 1 - d {
                ()
            } else if keep[0] == i + 1 - d {
                let mut keep = keep.clone();
                keep.remove(0);
                *next.entry(keep).or_insert(mi(0)) += val;
            } else if keep[0] > i + 1 + d {
                ()
            } else {
                for j in 0..keep.len() as i64 {
                    if keep[j as usize] >= i + 1 - d && keep[j as usize] <= i + 1 + d {
                        let mut keep = keep.clone();
                        keep.remove(j as usize);
                        *next.entry(keep).or_insert(mi(0)) += val;
                    }
                }
            }
    ...
    writer.ln(dp.get(&Vec::new()).unwrap_or(&mi(0)));
}
