...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let (rs, cs): (usize, usize) = reader.v2();
    let (rt, ct): (usize, usize) = reader.v2();
    let s = reader.bool_map(h, '#');
    let grid = Grid::new(h, w, s);
    // dp[direction][pos] = Dist
    let mut dp = vec![vec![1 << 30; h * w]; 4];
    let start = grid.key(cs - 1, rs - 1);
    dp[0][start] = 0;
    dp[1][start] = 0;
    dp[2][start] = 0;
    dp[3][start] = 0;
    ...
    heap.push((Reverse(0), (start, 2)));
    heap.push((Reverse(0), (start, 3)));
    while let Some((Reverse(d), (pos, dir))) = heap.pop() {
        if dp[dir][pos] != d {
            continue;
        }
        grid.edges(pos).iter().for_each(|e| {
            if !grid.get(e.dst) {
                return;
            }
            let to = if e.src == e.dst + 1 {
                0
            } else if e.src == e.dst + w {
                1
            } else if e.src + 1 == e.dst {
                2
            } else {
                3
            };
            let next = if to == dir { d } else { d + 1 };
            if chmin!(dp[to][e.dst], next) {
                heap.push((Reverse(next), (e.dst, to)))
            }
        })
    }
    let goal = grid.key(ct - 1, rt - 1);
    let mut ans = 1 << 30;
    for i in 0..4 {
        chmin!(ans, dp[i][goal]);
    }
