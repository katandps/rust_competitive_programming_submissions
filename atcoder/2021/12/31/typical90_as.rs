...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for p in 0..1 << n {
        ...
        for x in 0..n {
            ...
            for y in x + 1..n {
                ...
                chmax!(
                    m,
                    (xy[x].0 - xy[y].0) * (xy[x].0 - xy[y].0)
                        + (xy[x].1 - xy[y].1) * (xy[x].1 - xy[y].1)
                );
            }
        }
        dist[p] = m;
    ...
    // dp[i][p] = patterns : 残り集合pでi個のグループを作ったときの求める最小値
    ...
    for g in 0..k {
        for p in 1..1 << n {
            ...
            let mut next = p;
            while next > 0 {
                chmin!(dp[g + 1][p ^ next], max(dp[g][p], dist[next]));
                next -= 1;
                next &= p;
            }
        }
