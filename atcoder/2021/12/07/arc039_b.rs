...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (n, k): (usize, usize) = reader.v2();
    let mvt: ModValTable<Mi> = ModValTable::new(1000);
    if n <= k {
        let mut p = vec![0; n];
        for i in 0..k {
            p[i % n] += 1;
        ...
        for i in 0..n {
            *counts.entry(p[i]).or_insert(0) += 1;
        }
        let mut ans = mvt.factorial(n as i64);
        for (_, c) in counts {
            ans /= mvt.factorial(c);
        }
        writer.ln(ans)
    } else {
        writer.ln(mvt.factorial(n as i64 + k as i64 - 1)
            / mvt.factorial(n as i64 - 1)
            / mvt.factorial(k as i64))
    }
}
