...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut sums = vec![0];
    let mut cnts = vec![0usize];
    for i in 0..max + 1 {
        sums.push(sums[i] + sum[i]);

        cnts.push(cnts[i] + cnt[i]);
    }
    if k >= sums[max] {
        return writer.ln((sums[max] + k) / n);
    }

    let mut ans = 1;
    for g in 2..max {
        let (mut l, mut r, mut cost) = (1, g, 0i64);
        while l < max {
            let c = cnts[min(r, max)] - cnts[l];
            let s = sums[min(r, max)] - sums[l];
            cost += c as i64 * r as i64 - s as i64;
            l += g;
            r += g;
        }
        if cost <= k as i64 {
