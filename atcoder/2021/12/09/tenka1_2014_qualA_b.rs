...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let s = reader.chars();
    let mut damage_queue = BinaryHeap::new();
    let mut return_queue = BinaryHeap::new();
    let mut combo = 0;
    let mut total_damage = 0i64;
    let mut charge = -100;
    for i in 0..s.len() + 10 {
        while let Some((Reverse(ret), damage)) = damage_queue.pop() {
            if ret > i {
                damage_queue.push((Reverse(ret), damage));
                break;
            }
            total_damage += damage;
            combo += 1;
        ...
        while let Some(Reverse(ret)) = return_queue.pop() {
            if ret > i {
                return_queue.push(Reverse(ret));
                break;
            }
        }
        if i < s.len() && charge + 2 < i as i64 {
            if s[i] == 'N' && return_queue.len() < 5 {
                damage_queue.push((Reverse(i + 2), 10 + combo / 10));
                return_queue.push(Reverse(i + 7));
            } else if s[i] == 'C' && return_queue.len() < 3 {
                damage_queue.push((Reverse(i + 4), 5 * (10 + combo / 10)));
                return_queue.push(Reverse(i + 9));
                return_queue.push(Reverse(i + 9));
                return_queue.push(Reverse(i + 9));
                charge = i as i64;
            }
