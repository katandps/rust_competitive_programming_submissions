...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let mut diff = vec![None; n + 1];
    let mut queries = Vec::new();
    for _ in 0..q {
        let t: usize = reader.v();
        let (x, y): (usize, usize) = reader.v2();
        let v: i64 = reader.v();
        queries.push((t, x, y, v));
        if t == 0 {
            diff[x] = Some(v);
        }
    }
    ...
    for i in 0..n {
        if let Some(p) = diff[i] {
            potential[i + 1] = p - potential[i];
        }
    }
    for (t, x, y, v) in queries {
        if t == 0 {
            ...
            if uf.same(x, y) {
                if (x + y) % 2 == 0 {
                    writer.ln(potential[y] - potential[x] + v);
                } else {
                    writer.ln(potential[y] + potential[x] - v);
                }
            } else {
