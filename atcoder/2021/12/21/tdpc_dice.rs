...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    if d != 1 {
        return writer.ln(0.0);
    }
    // dp[2][3][5];
    let mut dp = vec![vec![vec![0.0; p[5] + 1]; p[3] + 1]; p[2] + 1];
    dp[0][0][0] = 1.0;
    for _ in 0..n {
        let mut next = vec![vec![vec![0.0; p[5] + 1]; p[3] + 1]; p[2] + 1];
        for two in 0..=p[2] {
            if dp[two][0][0] == 0.0 {
                break;
            }
            for three in 0..=p[3] {
                if dp[two][three][0] == 0.0 {
                    break;
                }
                for five in 0..=p[5] {
                    if dp[two][three][five] == 0.0 {
                        break;
                    }
                    next[two][three][five] += dp[two][three][five];
                    next[min(two + 1, p[2])][three][five] += dp[two][three][five];
                    next[two][min(three + 1, p[3])][five] += dp[two][three][five];
                    next[min(two + 2, p[2])][three][five] += dp[two][three][five];
                    next[two][three][min(five + 1, p[5])] += dp[two][three][five];
                    next[min(two + 1, p[2])][min(three + 1, p[3])][five] += dp[two][three][five];
                }
            }
        }
        dp = next;
    }
    writer.ln(dp[p[2]][p[3]][p[5]] / 6.0_f64.powf(n as f64));
}
