...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for s in 0..k {
        ...
        for i in 0..1 << (k - s - 1) {
            // lとrの試合が行われる確率p
            for l in i * num * 2..(i * num * 2) + num {
                for r in i * num * 2 + num..i * num * 2 + num * 2 {
                    let p = dp[s][l] * dp[s][r];
                    let (l_rate, r_rate) = (rates[l], rates[r]);
                    dp[s + 1][l] += p / (1.0 + 10.0_f64.powf((r_rate - l_rate) / 400.0));
                    dp[s + 1][r] += p / (1.0 + 10.0_f64.powf((l_rate - r_rate) / 400.0));
                }
            }
        }
    }
