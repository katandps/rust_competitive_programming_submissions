...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (h, w, m): (usize, usize, usize) = reader.v3();
    let xy = reader.vec2::<usize, usize>(m);

    let mut row_limit = vec![w; h];
    let mut col_limit = vec![h; w];
    ...
    let mut ans = 0;
    let mut lims = vec![Vec::new(); h + 1];

    for col in 0..w {
        if col < row_limit[0] {
            ans += col_limit[col] as i64;
            lims[col_limit[col]].push(col);
        } else {
            lims[0].push(col);
        }
    }
    let mut shade: BinaryIndexedTree<Addition<i64>> = BinaryIndexedTree::from(w + 1);
    for row in 0..col_limit[0] {
        ans += shade.sum(row_limit[row] - 1);
        for &lim in &lims[row] {
            shade.add(lim, 1);
        }
    }
