...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let front = knapsack(a.iter().take(20).cloned().collect::<Vec<_>>());
    let back = knapsack(a.iter().skip(20).cloned().collect::<Vec<_>>());
    let mut ans = 0;
    for i in 0..=min(k, 20) {
        let j = k - i;
        if j > 20 {
        ...
        for &f in &front[i] {
            ans += binary_search(-1, back[j].len() as i64, |m| back[j][m as usize] <= p - f) + 1;
        }
    }
    writer.ln(ans);
}

// ret[num] = vec[sum of price]
fn knapsack(a: Vec<i64>) -> Vec<Vec<i64>> {
    let mut ret = vec![Vec::new(); 21];
    ret[0].push(0);

    for i in 0..a.len() {
        for j in (0..ret.len()).rev() {
            for k in 0..ret[j].len() {
                let t = ret[j][k] + a[i];
                ret[j + 1].push(t);
            }
        }
