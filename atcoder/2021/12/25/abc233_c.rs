...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (n, x): (usize, usize) = reader.v2();
    let mut a = Vec::new();
    for _ in 0..n {
        let l: usize = reader.v();
        a.push(reader.vec::<usize>(l));
    }
    ...
    for v in a {
        ...
        for (&rest, &count) in &dp {
            for &a in &v {
                if rest % a == 0 {
                    *next.entry(rest / a).or_insert(0) += count;
                }
            }
        }
        dp = next;
    }
    writer.ln(dp.get(&1).unwrap_or(&0));
}
