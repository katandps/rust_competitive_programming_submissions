...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let c = reader.bool_map(h, '#');
    let g = Grid::new(h, w, c);
    let mut ng = 0;
    for i in 0..h * w {
        if !*g.get(i) {
            ng |= 1 << i;
    ...
    for i in 0..h * w {
        if ng >> i & 1 == 0 {
            chmax!(ans, f(i, i, 0, ng, &g));
        }
    }
    writer.ln(ans);
}

fn f(start: usize, pos: usize, cur: usize, ng: usize, g: &Grid<bool>) -> i32 {
    if cur.count_ones() >= 3 && pos == start {
        return cur.count_ones() as i32;
    }
    let mut ret = -1;
    for e in g.edges(pos) {
        if e.weight && cur >> e.dst & 1 == 0 {
            chmax!(ret, f(start, e.dst, cur | (1 << e.dst), ng, g));
        }
    }
    ret
}
