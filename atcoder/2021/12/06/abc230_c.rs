...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let _n: usize = reader.v();
    let a: usize = reader.v();
    let b: usize = reader.v();
    let (p, q, r, s): (usize, usize, usize, usize) = reader.v4();
    let mut ans = vec![vec![false; s - r + 1]; q - p + 1];
    for row in 0..q - p + 1 {
        for col in 0..s - r + 1 {
            let (y, x) = (col + r - 1, row + p - 1);
            if y + a == x + b || x + y + 2 == a + b {
                ans[row][col] = true;
            }
        }
    }
