...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let (mut start, mut goal) = (0, 0);
    for i in 0..h * w {
        if grid.get(i) == &'s' {
            start = i;
        } else if grid.get(i) == &'g' {
            goal = i;
        }
    }

    let mut dist = vec![1 << 30; h * w];
    ...
    while let Some((Reverse(d), src)) = heap.pop() {
        ...
        grid.edges(src).iter().for_each(|e| {
            let w = if grid.get(e.dst) == &'#' { 1 } else { 0 };
            if dist[e.dst] > dist[src] + w {
                dist[e.dst] = dist[src] + w;
                heap.push((Reverse(dist[e.dst]), e.dst))
            }
        });
    }

    writer.ln(if dist[goal] <= 2 { "YES" } else { "NO" });
}
