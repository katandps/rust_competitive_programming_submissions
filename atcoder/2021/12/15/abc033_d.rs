...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    for i in 0..n {
        ...
        for l in 0..v.len() as i64 {
            // 鋭角になる最も右の点
            let r1 = binary_search(l, l + v.len() as i64, |r| {
                v[l as usize].cross(v[r as usize % v.len()]) > 0
                    && v[l as usize].dot(v[r as usize % v.len()]) > 0
            });
            // 直角になる最も右の点
            let r2 = binary_search(l, l + v.len() as i64, |r| {
                v[l as usize].cross(v[r as usize % v.len()]) > 0
                    && v[l as usize].dot(v[r as usize % v.len()]) >= 0
            });
            // 180度未満の角をなす最も右の点
            let r3 = binary_search(l, l + v.len() as i64, |r| {
                v[l as usize].cross(v[r as usize % v.len()]) > 0
            });
            cnt[1] += r2 - r1;
            cnt[2] += r3 - r2;
    ...
    writer.ln(format!(
        "{} {} {}",
        n * (n - 1) * (n - 2) / 6 - cnt[1] as usize - cnt[2] as usize,
        cnt[1],
        cnt[2]
    ));
}
