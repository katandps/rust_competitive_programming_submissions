...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    let (h, w, k): (i64, i64, i64) = reader.v3();
    let (x1, y1, x2, y2): (i64, i64, i64, i64) = reader.v4();

    let mut cur = vec![mi(0); 4];
    match (x1 == x2, y1 == y2) {
        (true, true) => cur[0] = mi(1),
        (true, false) => cur[2] = mi(1),
        (false, true) => cur[1] = mi(1),
        (false, false) => cur[3] = mi(1),
    }

    for _ in 0..k {
        let mut next = vec![mi(0); 4];
        next[0] = cur[1] + cur[2];
        next[1] = cur[0] * (h - 1) + cur[1] * (h - 2) + cur[3];
        next[2] = cur[0] * (w - 1) + cur[2] * (w - 2) + cur[3];
        next[3] = cur[1] * (w - 1) + cur[2] * (h - 1) + cur[3] * (h + w - 4);
        cur = next;
    }
    writer.ln(cur[0]);
}
