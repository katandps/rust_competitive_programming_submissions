...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let q: usize = reader.val();
    const N: i64 = 1048576;
    let mut set = (0..N as i64).collect::<BTreeSet<_>>();
    let mut buf = vec![-1; N as usize + 1];
    for _ in 0..q {
        let t: usize = reader.val();
        let x: i64 = reader.val();
        if t == 1 {
            if let Some(&i) = set.range((x - 1) % N..).next() {
                set.remove(&i);
                buf[i as usize] = x
            } else {
                let i = *set.range(0..).next().unwrap();
                set.remove(&i);
                buf[i as usize] = x;
            }
        } else {
            writer.println(buf[((x - 1) % N) as usize]);
        }
    }
}
