...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let mut dp = vec![0; n];
    dfs(0, 0, &mut dp, &graph);

    let mut dp2 = vec![mi(0); n];
    dfs2(0, 0, &dp, &mut dp2, &mut PowTable::default(), &graph);
    let ans: Mi = dp2.into_iter().sum();
    writer.ln(ans);
}

fn dfs(cur: usize, parent: usize, dp: &mut Vec<i64>, g: &Graph<usize>) {
    let mut c = 1;
    for e in g.edges(cur) {
        if e.dst == parent {
            continue;
        }
        dfs(e.dst, e.src, dp, g);
        c += dp[e.dst];
    }
    dp[cur] = c;
}
...
fn dfs2(
    ...
    for e in g.edges(cur) {
        ...
        dfs2(e.dst, e.src, dp, dp2, powtable, g);
        childs.push(dp[e.dst]);
    }
    if dp[cur] < g.size() as i64 {
        childs.push(g.size() as i64 - dp[cur]);
    }
    let n = g.size() as i64;
    if childs.len() > 1 {
        let mut v = powtable.pow(-1); // 自分が白
        v -= powtable.pow(-n); // 全部白

        // 黒の子が一つだけ
        for &c in &childs {
            v -= powtable.pow(c - n) - powtable.pow(-n);
        }
        dp2[cur] = v;
    }
}
