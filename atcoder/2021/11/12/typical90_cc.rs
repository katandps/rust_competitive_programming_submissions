...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let k: usize = reader.val();
    let ab: Vec<(usize, usize)> = reader.vec2(n);
    ...
    let mut map = vec![vec![0i64; MAX + 1]; MAX + 1];

    for i in 1..=MAX {
        for j in 1..=MAX {
            map[i][j] = map[i - 1][j] + map[i][j - 1] - map[i - 1][j - 1] + count[i][j];
        }
    }
    ...
    for i in k + 1..=MAX {
        for j in k + 1..=MAX {
            chmax!(
                ans,
                map[i][j] + map[i - k - 1][j - k - 1] - map[i - k - 1][j] - map[i][j - k - 1]
            );
        }
    }
    writer.println(&ans);
