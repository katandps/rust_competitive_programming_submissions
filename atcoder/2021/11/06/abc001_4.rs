...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let n = reader.val();
    let a = reader.vec::<String>(n);
    let mut rainy = vec![false; 12 * 24 + 1];
    for ai in a {
        let v = ai
            .split("-")
            .map(|s| s.parse().unwrap())
            .collect::<Vec<i64>>();
        let l = v[0] / 100 * 12 + (v[0] % 100) / 5;
        let r = v[1] / 100 * 12 + (v[1] % 100 + 4) / 5;
        for i in l..r {
            rainy[i as usize] = true;
        }
    }
