...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let b = reader.vec::<i64>(n);

    // 0: 直前のA、Bを残している
    // 1: 直前のAを除いている
    // 2: 直前のBを除いている
    let mut dp0 = vec![vec![1 << 60; 4]; n + 1];
    let mut dp1 = vec![vec![1 << 60; 4]; n + 1];
    let mut dp2 = vec![vec![1 << 60; 4]; n + 1];
    let mut dp3 = vec![vec![1 << 60; 4]; n + 1];
    dp0[0][0] = 0;
    dp1[0][1] = 0;
    dp2[0][2] = 0;
    dp3[0][3] = 0;
    ...
    let mut ans = 1 << 60;
    chmin!(ans, s(dp0, n, &a, &b));
    chmin!(ans, s(dp1, n, &a, &b));
    chmin!(ans, s(dp2, n, &a, &b));
    chmin!(ans, s(dp3, n, &a, &b));
    writer.ln(ans);
}

fn s(mut dp: Vec<Vec<i64>>, n: usize, a: &Vec<i64>, b: &Vec<i64>) -> i64 {
    for i in 1..=n {
        dp[i][0] = min!(dp[i - 1][1], dp[i - 1][3]);
        dp[i][1] = min!(dp[i - 1][0], dp[i - 1][2], dp[i - 1][3]) + a[i % n];
        dp[i][2] = min!(dp[i - 1][0], dp[i - 1][2], dp[i - 1][3]) + b[(i + n - 1) % n];
