...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    for i in run {
        ...
        let left1 = bt.range(0..i).last();
        let right1 = bt.range(i + 1..).next();
        match (left1, right1) {
            (Some(l), Some(r)) => {
                let l2 = *bt.range(0..*l).last().unwrap_or(&0);
                let r2 = *bt.range(r + 1..).next().unwrap_or(&(n + 1));
                ans += p[i - 1] * ((l - l2) * (r - i) + (i - l) * (r2 - r));
            }
            (Some(l), _) => {
                let l2 = *bt.range(0..*l).last().unwrap_or(&0);
                ans += p[i - 1] * (l - l2) * (n + 1 - i);
            }
            (_, Some(r)) => {
                let r2 = *bt.range(r + 1..).next().unwrap_or(&(n + 1));
                ans += p[i - 1] * i * (r2 - r);
