...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (h, w, c, q): (usize, usize, usize, usize) = reader.v4();
    let tnc = reader.vec3::<usize, usize, usize>(q);
    ...
    for (t, n, c) in tnc.iter().rev() {
        if done.contains(&(t, n)) {
            continue;
        }
        done.insert((t, n));
        match t {
            1 => {
                *colors.entry(c).or_insert(0) += w - columns;
                rows += 1;
            }
            2 => {
                *colors.entry(c).or_insert(0) += h - rows;
                columns += 1;
            }
            _ => unreachable!(),
        }
    ...
    writer.join(
        &(1..=c)
            .map(|i| colors.get(&i).unwrap_or(&0))
            .collect::<Vec<_>>(),
        " ",
    )
}
