...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let a = reader.digits();
    let b = reader.digits();
    if a.iter().max().unwrap() + b.iter().max().unwrap() < 10 {
        writer.join(&a, "");
        writer.join(&b, "");
        return;
    }
    ...
    if ans[ans.len() - 1].0 + ans[ans.len() - 1].1 > 9 {
    ...
    writer.join(&a_ans.iter().rev().collect::<Vec<_>>(), "");
    writer.join(&b_ans.iter().rev().collect::<Vec<_>>(), "");
}
