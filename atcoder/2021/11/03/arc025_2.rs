...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (h, w) = (reader.val::<usize>(), reader.val::<usize>());
    let mut a = reader.matrix::<i64>(h, w);
    for r in 0..h {
        for c in 0..w {
            if (r + c) % 2 == 1 {
                a[r][c] = -a[r][c];
    ...
    for r in 0..h {
        for c in 0..w {
            sum[r + 1][c + 1] = sum[r][c + 1] + sum[r + 1][c] + a[r][c] - sum[r][c];
        }
    }
    let mut ans = 0;
    for from_r in 0..h {
        for from_c in 0..w {
            for to_r in from_r + 1..h + 1 {
                for to_c in from_c + 1..w + 1 {
                    let k = sum[to_r][to_c] + sum[from_r][from_c]
                        - sum[from_r][to_c]
                        - sum[to_r][from_c];
                    if k == 0 {
                        chmax!(ans, (to_r - from_r) * (to_c - from_c));
                    }
                }
            }
        }
