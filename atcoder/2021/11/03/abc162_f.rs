...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    // dp[i][x][b]
    let mut dp = vec![vec![vec![-1 << 60; 2]; 4]; n + 1];

    dp[0][0][0] = 0;
    for i in 0..n {
        for x in 0..3 {
            chmax!(dp[i + 1][x + 1][0], dp[i][x][0]);
        }
        for x in 0..3 {
            chmax!(dp[i + 1][x][1], dp[i][x][0] + a[i]);
            chmax!(dp[i + 1][x][0], dp[i][x][1]);
        }
    }
    for x in 0..3 {
        chmax!(dp[n][x + 1][0], dp[n - 1][x][1]);
    }
    let ans = if n % 2 == 0 {
        max!(dp[n][1][0], dp[n][1][1])
    } else {
        max!(dp[n][2][0], dp[n][2][1], dp[n][1][0], dp[n][1][1])
    };
    writer.println(ans);
}
