...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let (n, k): (usize, i128) = reader.val2();
    let a: Vec<i128> = reader.vec(n);

    let ans = binary_search(0, 1_000_000_000_000_000_010, |mid| {
        let mut t = 0;
        for &ai in &a {
            t += min(ai, mid);
        }
...
pub fn binary_search<F: Fn(i128) -> bool>(mut ok: i128, mut ng: i128, f: F) -> i128 {
    while (ok - ng).abs() > 1 {
        let mid = (ok + ng) / 2;
        if f(mid) {
            ok = mid
