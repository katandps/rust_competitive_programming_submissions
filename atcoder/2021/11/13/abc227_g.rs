...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    ...
    let k = reader.val::<i64>();
    let k = min(k, n - k);
    let sq = max(sqrt(n), k);
    let primes = primes(sq as usize);

    let mut ans = mi(1);

    for &p in &primes {
        ans *= div(n, p) - div(n - k, p) - div(k, p) + 1;
    }
    let mut nums = (n - k + 1..=n).collect::<Vec<_>>();
    for &p in &primes {
        for i in 0.. {
            let j = ((n - k + p) / p * p + p * i - (n - k + 1)) as usize;
            if j >= nums.len() {
                break;
            }
            while nums[j] % p == 0 {
