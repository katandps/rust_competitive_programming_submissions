...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for i in 0..s.len() {
        road.insert((dx + 1000000, dy + 1000000));
        match s[i] {
            'L' => dx -= 1,
            'R' => dx += 1,
            'U' => dy -= 1,
    ...
    if dx < 0 {
        dx = -dx;
        road = road.into_iter().map(|(x, y)| (-x, y)).collect();
    }
    if dy < 0 {
        dy = -dy;
        road = road.into_iter().map(|(x, y)| (x, -y)).collect();
    ...
    for &(x, y) in &road {
        if dx != 0 {
            let a = x / dx;
            set.entry((x % dx, y - a * dy))
                .or_insert(BTreeSet::new())
                .insert(a);
        } else {
            let a = y / dy;
            set.entry((x - a * dx, y % dy))
                .or_insert(BTreeSet::new())
                .insert(a);
        }
    }
    ...
    let mut ans = 0;
    for (_p, s) in &set {
        let sv = s.iter().collect_vec();
        ans += k;
        for i in 1..sv.len() {
            ans += min(k, sv[i] - sv[i - 1]);
        }
    }
