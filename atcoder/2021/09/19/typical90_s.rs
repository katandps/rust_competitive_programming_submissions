...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", f(0, 2 * n, &a, &mut dp));
}
const INF: i64 = 1 << 60;

fn f(l: usize, r: usize, a: &Vec<i64>, dp: &mut Vec<Vec<i64>>) -> i64 {
    if dp[l][r] != INF {
        return dp[l][r];
    }
    if r - l == 2 {
        return (a[r - 1] - a[l]).abs();
    }
    let mut ret = f(l + 1, r - 1, a, dp) + (a[r - 1] - a[l]).abs();
    for i in 1..(r - l) / 2 {
        chmin!(ret, f(l, l + i * 2, a, dp) + f(l + i * 2, r, a, dp));
    }
    dp[l][r] = ret;
    ret
}
