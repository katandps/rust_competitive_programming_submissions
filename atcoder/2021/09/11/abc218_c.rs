...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let n = reader.u();
    let s = reader.bool_map(n, '.');
    let mut t = reader.bool_map(n, '.');

    let s_count = s.iter().fold(0, |x, s| {
        x + s.iter().fold(0, |x, &b| x + if b { 1 } else { 0 })
    });
    let t_count = t.iter().fold(0, |x, t| {
        x + t.iter().fold(0, |x, &b| x + if b { 1 } else { 0 })
    });
    if s_count != t_count {
        println!("{}", "No");
        return;
    ...
    if mat(&s, &t, s_count, n as i64) {
        println!("{}", "Yes");
        return;
    }

    let mut t2 = vec![vec![false; n]; n];
    for i in 0..n {
        for j in 0..n {
            t2[n - 1 - j][i] = t[i][j];
        }
    }
    if mat(&s, &t2, s_count, n as i64) {
        println!("{}", "Yes");
        return;
    }
...
fn mat(s: &Vec<Vec<bool>>, t: &Vec<Vec<bool>>, cnt: usize, n: i64) -> bool {
    for ydiff in -(n - 1)..=n - 1 {
        'case: for xdiff in -(n - 1)..=n - 1 {
            let mut count = 0;
            for y in 0..n - ydiff.abs() {
                for x in 0..n - xdiff.abs() {
                    let addy = (y + ydiff.abs()) as usize;
                    let addx = (x + xdiff.abs()) as usize;
                    let (x, y) = (x as usize, y as usize);
                    if xdiff < 0 && ydiff < 0 {
                        if s[y][x] != t[addy][addx] {
                            continue 'case;
                        }
                        if s[y][x] {
                            count += 1;
