...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    let (h, w) = reader.u2();
    let hw = reader.matrix(h, w);

    const MOD: i64 = 1_000_000_007;
    let mut dp = vec![1; h * w];

    let grid = Grid::new(h, w, hw);
    let mut heap = BinaryHeap::new();
    for y in 0..h {
        for x in 0..w {
            let key = grid.key(x, y);
            heap.push((Reverse(grid.get(key)), key));
        }
    }

    while let Some((Reverse(a), key)) = heap.pop() {
        for to in grid.neighbor(key) {
            if grid.get(to) > a {
                dp[to] += dp[key];
                dp[to] %= MOD;
            }
