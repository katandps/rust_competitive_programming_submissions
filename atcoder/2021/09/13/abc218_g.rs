...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    println!("{}", f(0, 0, &a, &g, &mut trie));
}
const INF: i64 = 1 << 58;
fn f(cur: usize, parent: usize, a: &Vec<i64>, g: &Graph, btrie: &mut BinaryTrie) -> i64 {
    btrie.insert(a[cur]);
    let l = btrie.size();
    let edges = g.edges_from(cur);
    let ret = if edges.len() == 1 && edges[0].dst == parent {
        if l % 2 == 1 {
            btrie.nth(l / 2)
        } else {
            (btrie.nth(l / 2) + btrie.nth(l / 2 - 1)) / 2
        }
    } else {
        let mut ret = INF;
        if l % 2 == 1 {
