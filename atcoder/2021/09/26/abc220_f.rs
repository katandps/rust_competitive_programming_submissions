...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let _ = f(1, 1, &g, &mut dp);
    let mut ans = vec![0; n + 1];
    f2(n, 1, 1, &g, &dp, &mut ans);
    for i in 1..=n {
        println!("{}", ans[i]);
    }
}

fn f(src: usize, par: usize, g: &Vec<Vec<usize>>, dp: &mut Vec<(usize, usize)>) -> (usize, usize) {
    let mut num = 0;
    let mut sum = 0;
    for &dst in &g[src] {
        if par == dst {
...
fn f2(
    ...
    if src == par {
        ...
        let par_num = n - dp[src].0 - 1;
        let my_sum = dp[src].1 + 1 + dp[src].0;
        ans[src] = ans[par] + dp[src].1 + par_num - my_sum;
    }
    for &dst in &g[src] {
        if par == dst {
            continue;
        }
        f2(n, dst, src, g, dp, ans);
    }
}
