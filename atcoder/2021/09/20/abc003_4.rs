...
use crate::mod_val_table::ModValTable;
#[allow(unused_imports)]
use mod_int::*;
...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mvt = ModValTable::new(1000);
    let mut dp = vec![vec![None; 31]; 31];
    let mut ans = f(x, y, d + l, &mvt, &mut dp) * (r as i64 - x + 1) * (c as i64 - y + 1);
    if d > 0 {
        ans /= mvt.factorial(d);
    }
    if l > 0 {
...
fn f(x: i64, y: i64, t: i64, mvt: &ModValTable<Mi>, dp: &mut Vec<Vec<Option<Mi>>>) -> Mi {
    if let Some(r) = dp[y as usize][x as usize] {
        return r;
    }
    let mut all = mvt.permutation(x * y, t);
    for xx in 1..=x {
        for yy in 1..=y {
            if xx == x && yy == y {
                continue;
            }
            all -= f(xx, yy, t, mvt, dp) * (x - xx + 1) * (y - yy + 1);
        }
    }
    dp[y as usize][x as usize] = Some(all);
    all
}
