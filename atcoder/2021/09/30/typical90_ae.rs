...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut output: Writer<W>) {
    ...
    // grundy[w][b] = mex
    let mut grundy = vec![vec![-1; 1326]; 51];
    let mut xor = 0;
    for i in 0..n {
        let k = f(w[i], b[i], &mut grundy);
        xor ^= k;
    }

    output.println(if xor == 0 { &"Second" } else { &"First" });
}

fn f(w: usize, b: usize, grundy: &mut Vec<Vec<i64>>) -> i64 {
    if grundy[w][b] >= 0 {
        return grundy[w][b];
    }
    let mut v = Vec::new();
    if w > 0 {
        v.push(f(w - 1, b + w, grundy) as usize);
    }
    for i in 1..=b / 2 {
        v.push(f(w, b - i, grundy) as usize);
    }
    v.sort();
    v.dedup();
    grundy[w][b] = (0..).find(|i| Some(i) != v.get(*i)).unwrap_or(v.len()) as i64;
    grundy[w][b]
}
