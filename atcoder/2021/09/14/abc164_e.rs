...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    for _ in 0..m {
        let (u, v, a, b) = reader.u4();
        g[u - 1].push((v - 1, a, b));
        g[v - 1].push((u - 1, a, b));
    }

    let limit: usize = 50 * n;
    let cd = reader.uv2(n);

    let mut dp = vec![vec![INF; limit + 1]; n];
    dp[0][min(limit,s)] = 0;

    // heap.pop = (Reverse(dist), (town, silver));
    let mut q = BinaryHeap::new();
    q.push((Reverse(0), (0, min(limit, s))));

    while let Some((Reverse(dist), (src, silver))) = q.pop() {
        let (c, d) = cd[src];
        if dp[src][silver] != dist {
            continue;
        }
        let to = min(limit, silver + c);
        if dp[src][to] > dp[src][silver] + d {
            dp[src][to] = dp[src][silver] + d;
            q.push((Reverse(dp[src][to]), (src, to)));
        }
        for &(dst, cost, time) in &g[src] {
            if silver < cost {
                continue;
            }
            let to = silver - cost;
            if dp[dst][to] > dp[src][silver] + time {
                dp[dst][to] = dp[src][silver] + time;
                q.push((Reverse(dp[dst][to]), (dst, to)));
            }
        }
    }
    for t in 1..n {
