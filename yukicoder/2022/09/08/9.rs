...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let b = reader.vec::<usize>(n);
    let mut ans = 1 << 60;
    for i in 0..n {
        chmin!(ans, simulator(&a, &b, i));
    }
    writer.ln(ans);
}

fn simulator(a: &Vec<usize>, b: &Vec<usize>, start: usize) -> usize {
    let mut heap = BinaryHeap::new();
    for &ai in a {
        heap.push((Reverse(ai), Reverse(0)));
    }
    for i in 0..a.len() {
        let i = (i + start) % a.len();
        if let Some((Reverse(level), Reverse(times))) = heap.pop() {
            heap.push((Reverse(level + b[i] / 2), Reverse(times + 1)));
        }
    }
    let mut ans = 0;
    while let Some((Reverse(_level), Reverse(times))) = heap.pop() {
        chmax!(ans, times);
    }
    ans
}
