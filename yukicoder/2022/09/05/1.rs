...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let c: usize = reader.v();
    let v: usize = reader.v();
    let s = reader.vec::<usize>(v);
    let t = reader.vec::<usize>(v);
    let y = reader.vec::<usize>(v);
    let m = reader.vec::<usize>(v);
    // dp[i][r] = min // i番目の街にいて、r円残ってる時の最短時間
    let mut dp = vec![vec![1 << 60; c + 1]; n + 1];
    dp[1][c] = 0;

    let mut graph = Graph::new(n + 1);
    for i in 0..v {
        graph.add_arc(s[i], t[i], (y[i], m[i]));
    }

    for i in 1..n {
        for (dst, (y, m)) in graph.edges(i) {
            for r in y..=c {
                chmin!(dp[dst][r - y], dp[i][r] + m);
            }
        }
    }
    // dbg!(&dp);
