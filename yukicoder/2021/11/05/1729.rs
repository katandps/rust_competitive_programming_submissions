...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let d = reader.digits();
    let mut ans = vec![0; 8];
    let mut right = d.len() as i64 - 1;
    while right >= 0 {
        let mut p = 0;
        for i in (right - 2..=right).rev() {
            if i < 0 {
                continue;
            }
            let mut k = d[i as usize] + 10;
            if right - i > 0 {
                k *= 16;
            }
            if right - i > 1 {
    ...
    for k in 0..=7 {
        if ans[k] > 0 {
            btm.entry(Reverse(ans[k])).or_insert(Vec::new()).push(k);
        }
    }
    writer.print_join(&btm.iter().next().unwrap().1, " ");
}
