...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for _ in 0..n {
        ...
        let mut l_sum = 0;
        let mut l_minus = 0;
        let mut l_q = VecDeque::new();
        for i in (0..k - 1).rev() {
            l_sum += a[i];
            chmin!(l_minus, l_sum);
            if l_sum > 0 {
                l_q.push_back((l_minus, l_sum));
                l_sum = 0;
                l_minus = 0;
            }
        }
        l_q.push_back((l_minus, 1 << 50));
        let mut r_sum = 0;
        let mut r_minus = 0;
        let mut r_q = VecDeque::new();
        for i in k..n {
            r_sum += a[i];
            chmin!(r_minus, r_sum);
            if r_sum > 0 {
                r_q.push_back((r_minus, r_sum));
                r_sum = 0;
                r_minus = 0;
            }
        }
        r_q.push_back((r_minus, 1 << 50));
        let mut clear = false;
        loop {
            let mut get = false;
            if let Some((minus, gain)) = l_q.pop_front() {
                if cur + minus < 0 {
                    l_q.push_front((minus, gain));
                } else {
                    get = true;
                    cur += gain;
                }
            ...
            if let Some((minus, gain)) = r_q.pop_front() {
                if cur + minus < 0 {
                    r_q.push_front((minus, gain));
                } else {
                    get = true;
                    cur += gain;
                }
        ...
        writer.ln(if clear { "YES" } else { "NO" });
    }
}
