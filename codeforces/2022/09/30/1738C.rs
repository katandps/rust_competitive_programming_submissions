...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    for _ in 0..n {
        ...
        let mut eo = vec![0; 2];
        for ai in a {
            eo[ai.rem_euclid(2) as usize] += 1;
        }
        writer.ln(if s(eo[1], eo[0], &mut map).0 {
            "Alice"
        } else {
            "Bob"
        });
...
// aliceが(even, odd)のどちらにするか選べるかどうか
fn s(o: usize, e: usize, map: &mut HashMap<(usize, usize), (bool, bool)>) -> (bool, bool) {
    if let Some(r) = map.get(&(o, e)) {
        return *r;
    }
    if o + e <= 1 {
        map.insert((o, e), (o != 1, o == 1));
        return (o != 1, o == 1);
    }

    let (mut can_even, mut can_odd) = (false, false);
    // alice take odd
    if o >= 1 {
        let (mut re, mut ro) = (true, true);
        if o >= 2 {
            // bob take odd
            re &= s(o - 2, e, map).1;
            ro &= s(o - 2, e, map).0;
    ...
    map.insert((o, e), (can_even, can_odd));
    (can_even, can_odd)
}
