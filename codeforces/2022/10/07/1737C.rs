...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    'case: for _ in 0..reader.v::<usize>() {
        let n = reader.v::<usize>();
        let rc = reader.vec2::<usize, usize>(3);
        let (x, y) = reader.v2::<usize, usize>();

        let r = if rc[0].0 == rc[1].0 {
            rc[0].0
        } else if rc[0].0 == rc[2].0 {
            rc[0].0
        } else {
            rc[1].0
        };
        let c = if rc[0].1 == rc[1].1 {
            rc[0].1
        } else if rc[0].1 == rc[2].1 {
            rc[0].1
        } else {
            rc[1].1
        };
        dbg!(((x, y, r, c), rc[0], rc[1], rc[2]));

        let corner = vec![(1, 1), (1, n), (n, 1), (n, n)];
        for (c_r, c_c) in corner {
            if r == c_r && c == c_c {
                writer.ln(if x == r || y == c { "YES" } else { "NO" });
                continue 'case;
            }
        }
        writer.ln(if r % 2 != x % 2 && y % 2 != c % 2 {
            "NO"
        } else {
            "YES"
        });
