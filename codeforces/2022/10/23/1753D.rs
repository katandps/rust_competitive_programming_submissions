...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    ...
    let (p, q) = reader.v2::<i64, i64>();
    let beach = reader.char_map(n);
    const INF: i64 = 1 << 60;
    let mut map = vec![vec![INF; m]; n];
    ...
    while let Some((Reverse(cost), (i, j))) = pq.pop() {
        ...
        if i >= 2 {
            if beach[i - 2][j] == 'U' {
                if chmin!(map[i - 2][j], cost + q) {
                    pq.push((Reverse(map[i - 2][j]), (i - 2, j)));
                }
            }
        }
        if i + 2 < n {
        ...
        if j >= 2 {
            if beach[i][j - 2] == 'L' {
                if chmin!(map[i][j - 2], cost + q) {
                    pq.push((Reverse(map[i][j - 2]), (i, j - 2)));
                }
            }
        }
        if j + 2 < m {
        ...
        if i >= 1 && j >= 1 {
            if beach[i - 1][j - 1] == 'L' || beach[i - 1][j - 1] == 'U' {
                if chmin!(map[i - 1][j - 1], cost + p) {
                    pq.push((Reverse(map[i - 1][j - 1]), (i - 1, j - 1)));
                }
            }
        }
        if i >= 1 && j < m - 1 {
            if beach[i - 1][j + 1] == 'R' || beach[i - 1][j + 1] == 'U' {
                if chmin!(map[i - 1][j + 1], cost + p) {
                    pq.push((Reverse(map[i - 1][j + 1]), (i - 1, j + 1)));
                }
            }
        ...
        if i < n - 1 && j >= 1 {
            if beach[i + 1][j - 1] == 'L' || beach[i + 1][j - 1] == 'D' {
                if chmin!(map[i + 1][j - 1], cost + p) {
                    pq.push((Reverse(map[i + 1][j - 1]), (i + 1, j - 1)));
                }
            }
        ...
        if i < n - 1 && j < m - 1 {
            if beach[i + 1][j + 1] == 'R' || beach[i + 1][j + 1] == 'D' {
                if chmin!(map[i + 1][j + 1], cost + p) {
                    pq.push((Reverse(map[i + 1][j + 1]), (i + 1, j + 1)));
                }
            }
    ...
    for i in 0..n {
        for j in 0..m {
            if i < n - 1 {
                chmin!(ans, map[i][j] + map[i + 1][j]);
            }
            if j < m - 1 {
                chmin!(ans, map[i][j] + map[i][j + 1]);
            }
        }
    }
    if ans == INF {
