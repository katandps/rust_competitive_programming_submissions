...
pub fn solve<R: BufRead, W: Write>(mut reader: Reader<R>, mut writer: Writer<W>) {
    let t = reader.val::<usize>();
    'case: for _ in 0..t {
        let (n, m) = (reader.val::<i64>(), reader.val::<i64>());
        let s = reader.chars();
        let (mut x, mut y) = (0, 0);
        let mut lrud = vec![0, 0, 0, 0];
        let mut ans = (1, 1);
        for si in s {
            match si {
                'L' => {
            ...
            if lrud[0] + lrud[1] >= m || lrud[2] + lrud[3] >= n {
                writer.println(format!("{} {}", ans.0, ans.1));
                continue 'case;
            }
            ans = (lrud[2] + 1, lrud[0] + 1);
        }
        writer.println(format!("{} {}", ans.0, ans.1));
    }
}
