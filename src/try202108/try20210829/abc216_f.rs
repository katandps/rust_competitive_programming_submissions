...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let c = (0..n).sorted_by(|&i, &j| a[j].cmp(&a[i])).collect_vec();

    const MAX_PARAM: i64 = 5010;
    let mut dp = vec![vec![mi(0); MAX_PARAM as usize + 1]; n + 1];
    dp[n][0] = mi(1);
    for i in (0..n).rev() {
        dp[i] = dp[i + 1].clone();
        for from in 0..MAX_PARAM {
            let k = dp[i + 1][from as usize];
            dp[i][min((from + b[c[i]]) as usize, MAX_PARAM as usize)] += k;
        }
    }
