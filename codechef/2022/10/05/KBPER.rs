...
pub fn solve<R: BufRead, W: Write, F: FnMut() -> R>(mut reader: Reader<F>, mut writer: Writer<W>) {
    for _ in 0..reader.v::<usize>() {
        ...
        // dp[i][k][e][b] = i番目まで見てグループがk個、evenが残りe個でパリティがb
        let mut dp = vec![vec![vec![vec![mi(0); 2]; n + 1]; n + 1]; n + 1];
        ...
        if p[0] > 0 {
            ...
            dp[1][1][even][1] += (rest - even) as i64;
            if even > 0 {
                dp[1][1][even - 1][0] += even as i64;
            }
        }
        // dbg!(&dp[1], rest, even);
        if p[0] == 0 {
            ...
            for k in 0..=i {
                for e in 0..=rest {
                    ...
                    if p[i] == 0 {
                        dbg!(i, k, e, even, odd);
                        // even + odd
                        dp[i + 1][k + 1][e][1] += even * (rest - e) as i64;
                        // odd + odd
                        dp[i + 1][k][e][1] += odd * (rest - e) as i64;
                        dbg!(odd * (rest - e) as i64, rest, e);
                        if e > 0 {
                            // even + even
                            dp[i + 1][k][e - 1][0] += even * e as i64;
                            // odd + even
                            dp[i + 1][k + 1][e - 1][0] += odd * e as i64;
                        }
                    } else {
                        if p[i] % 2 == 1 {
                            // + odd
        ...
        // dbg!(&dp);
        writer.ln(dp[n][k][0][0] + dp[n][k][0][1]);
    }
}
