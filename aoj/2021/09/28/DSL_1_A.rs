...
fn main() {
    ...
    solve(Reader::new(stdin.lock()), &mut output);
    let out = stdout();
...
pub fn solve<R: BufRead>(mut reader: Reader<R>, output: &mut Vec<String>) {
    let (n, q) = reader.u2();
    let mut uf = UnionFind::new(n);
    for _ in 0..q {
        if reader.u() == 0 {
            uf.unite(reader.u(), reader.u());
        } else {
            if uf.root(reader.u()) == uf.root(reader.u()) {
