...
pub mod graph {
    ...
    impl Graph {
        ...
        /// lを始点とする各点までの経路数を求める
        /// グラフがDAGの場合に使用可
        pub fn path(&self, l: usize) -> Vec<Vec<usize>> {
            let list = self.topological_sort();
            // dp[i][length] = p
            let mut dp = vec![vec![0; self.v()]; self.v()];
            dp[0][l] = 1;
            for src in list {
                for e in self.edges_from(src) {
                    for len in 1..self.v() {
                        dp[len][e.dst] += dp[len - 1][src];
                    }
                }
            }
            dp
...
pub fn solve<R: BufRead>(mut reader: Reader<R>) {
    ...
    let mut gb = Graph::new(n);
    for &(u, v) in &uv {
        g.add_arc(u - 1, v - 1, 1);
        gb.add_arc(v - 1, u - 1, 1);
    }

    let go = g.path(0);
    let back = gb.path(n - 1);

    for i in 0..n {
        let mut ans = 0;
        for l in 0..n {
            for r in 0..n {
                ans += go[l][i] * back[r][i] * (l + r);
            }
        }
        println!("{}", ans);
    }
